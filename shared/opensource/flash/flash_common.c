...
void flash_init_nand_info(NVRAM_DATA *nvRam, FLASH_ADDR_INFO *fInfo)
{
    ...
    fInfo->flash_rootfs_max_size = 
		((nvramData.ulNandPartOfsKb[NP_DATA]/ulBlockSizeKb) - totalBlks/2 - 1 - 1) * flash_get_sector_size(0);
	#endif
...
*/
	psiStartAddr = (fInfo->flash_meta_start_blk - 1)*flash_get_sector_size(0);
    ...
    /* Record the bottum half first available block on nvRam->ulNandPartOfsKb[NP_ROOTFS_2] */	
	#if (defined(MSTC_FWUP_FROM_FLASH) || defined(CONFIG_BUILD_MSTC_FWUP_FROM_FLASH))
	/* Add firmware upgrade partition */
	...
	nvramData.ulNandPartSizeKb[NP_ROOTFS_2] = fInfo->flash_persistent_blk_offset/1024 - nvramData.ulNandPartOfsKb[NP_ROOTFS_2];
	#endif
	...
	{
#if 1
		/* BRCM_COMMON_BUGFIX: merge from http://svn.mitrastar.com.tw/svn/CPE_TRUNK/BRCM_412/Products/DSL-491HNU-B1B_V2_TTNET/dev_L06/shared, r15910 */
		/*In the CPE first boot up time, the Nand flash partition information will be written in Flash Block 0.The Bootloader data will be lost if cut the CPE power in that moment.*/
		cfe_usleep(2000000);
#endif
...
#ifdef _CFE_ /* Image Default, __CHT__, MitraStar Curtis, 20111124. */
