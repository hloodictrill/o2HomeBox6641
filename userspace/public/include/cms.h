...
typedef enum
{
   ...
   CMSRET_DU_UPDATE_VERSION_EXISTED= 9032,/**< Invalid Deployment Unit Update, version already exist
                                                    * (associated with ChangeDUState:update)
                                                    */
/*KeyYang@MSTC: Merge from Telefonica_Common, 20120712*/ 
#ifdef MSTC_COMMON_PATCH
   /*__MSTC__, Nelson Lee, Login Privilege */
...
/*__VERIZON__, Autumn*/
  ...
  CMSRET_RULE_ALREADYEXIST = 9858, 		 /*Jessica@MSTC: For GUI show error message requirement. 20130814*/
  CMSRET_SET_NOT_SUPPORTED_PARAM = 9859, 	/*[IreneWang@MSTC:TO2]$ Reject to write not function support parameter, 20140102*/
  CMSRET_CLIENT_ACCESS_GUI_NOT_IN_MACFILTER = 9860,
#endif
} CmsRet;
...
/*Created a shared memory for backup default X_5067F0_DECT object,
	KeyYang@MSTC 20130327 */
#ifdef STOLLMANN_CALLMNGR
#define CMS_DECT_DEV_FXS_TYPE_LEN			20
#define CMS_DECT_DEV_MSN_LEN				38
#define CMS_DECT_DEV_INTRN_CALL_NUM_LEN		10
#define CMS_DECT_DEV_INTRN_CALLING_NUM_LEN	50
#define CMS_DECT_DEV_INTRN_CALLED_NUM_LEN	50
#define CMS_DECT_DEV_INTRN_USED_CODEC_LEN	50
#define CMS_DECT_DEV_NAME_LEN				50

#define CMS_DECT_BS_NAME_LEN				13
#define CMS_DECT_BS_FW_VERSION_LEN			10
#define CMS_DECT_BS_EEPROM_VERSION_LEN		10
#define CMS_DECT_BS_HW_VERSION_LEN			10
#define CMS_MAX_DECT_DEV_NUM				6

typedef struct cmsDectDevice_s{
	char pcFxsType[CMS_DECT_DEV_FXS_TYPE_LEN + 1];
	char pcMsn[CMS_DECT_DEV_MSN_LEN + 1];
	char pcInternalCallPhoneNumber[CMS_DECT_DEV_INTRN_CALL_NUM_LEN + 1];
	unsigned int uiUseAccount;
	int bCallWaitingOn;
	unsigned int uiDialTimeout;
	int iConnectionState;
	int iBChannel;
	int iHookState;
	unsigned int bCallForwardingActive;
	int iConnectionStartTimestamp;
	char pcCallingNumber[CMS_DECT_DEV_INTRN_CALLING_NUM_LEN + 1];
	char pcCalledNumber[CMS_DECT_DEV_INTRN_CALLED_NUM_LEN + 1];
	char pcUsedCodec[CMS_DECT_DEV_INTRN_USED_CODEC_LEN + 1];
	int iRxGainFactor;
	int iTxGainFactor;
	int iLocalDeviceId;
	char pcName[CMS_DECT_DEV_NAME_LEN + 1];
	int bEntryInUse;
} cmsDectDevice_s;

typedef struct _cmsCommonDect_s{
	int bDectOnOff;
	int bEncryptionOnOff;
	char pcBaseName[CMS_DECT_BS_NAME_LEN + 1];
//	unsigned int uiBasePIN;
	char uiBasePIN[16];
	int bNemoOnOff;
	int bECOModeOnOff;
	char pcBaseFirmwareVersion[CMS_DECT_BS_FW_VERSION_LEN + 1];
	char pcBaseEEPROMVersion[CMS_DECT_BS_EEPROM_VERSION_LEN + 1];
	char pcBaseHardwareVersion[CMS_DECT_BS_HW_VERSION_LEN + 1];
	int iBaseStatus;
} cmsCommonDect_s;

typedef struct _cmsDectCfg_s{
	unsigned int 		locked:1 ;
	cmsCommonDect_s		dectCfg ;
	unsigned int 		numOfDev ;	
	cmsDectDevice_s		dectDevsCfg[CMS_MAX_DECT_DEV_NUM] ;
}cmsDectCfg_s;
#endif	/*end of STOLLMANN_CALLMNGR*/
