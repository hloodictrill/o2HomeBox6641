...
#include "pppoe.h"
#ifdef MSTC_COMMON_PATCH /* Follow 11n model to support pppoe log. __TELEFONICA__, MSTC Stan Su, 20120220. */
#include "oal.h"
#endif

#if 1 /*__MSTC__, chris 2012-11-02*/
#include "cms.h"
...
#endif /*__MSTC__, chris 2012-11-02*/
...
int session_disconnect(struct session *ses){
    ...
    ses->state = PADO_CODE;
#ifdef MSTC_COMMON_PATCH /* Follow 11n model to support pppoe log. __TELEFONICA__, MSTC Stan Su, 20120220. */
    oalLog_syslog(LOG_LEVEL_NOTICE, "Send PADT");
#endif
    return 0;
...
//    struct pppoe_tag *tags = NULL;
	...
	fd_set in;
	struct timeval tv;
	#if 1 /*__MSTC__, chris 2012-11-19, wait 3 seconds for lastest PADI */	
	bool DiscoverChange = TRUE; 
	UBOOL8 systemErr = FALSE;
	UBOOL8 ppp2DiscoverFail = TRUE;
	int Stage = 0;
	bool ppp1DiscoverFail=TRUE;
	Stage = getActivatedStatus(ACTIVATED_STATUS_STAGE);	
	#endif
	
    while((ses->retransmits < ses->retries || ses->retries==-1) && !systemErr){	
	#if 0 /*__MSTC__, chris 2012-11-19, wait 3 seconds for lastest PADI */
	fd_set in;
	struct timeval tv;
	#endif
	FD_ZERO(&in);
	...
	if(ses->retransmits>=0){
	    ++ses->retransmits;
	#if 1 /*__MSTC__, chris 2012-11-19*/
		if(!strcmp(req_name, PPP2IFNAME))
		{
			if(getActivatedStatus(ACTIVATION_IS_BBI_ACTIVATED) == 0)
			{
				tv.tv_sec = 3;
			}
			else
			{
				if(ses->retransmits > PPP_MAX_RETRY_INDEX)
				{
					ses->retransmits = PPP_REPEAT_RETRY_STARTINDEX;
				}
				if(ses->retransmits > 1)
				{
					devCtl_boardIoctl(BOARD_IOCTL_LED_CTRL, 0, "",  kLedInternet, kLedStateOff, "");
					devCtl_boardIoctl(BOARD_IOCTL_LED_CTRL, 0, "",  kLedInternet, kLedFailStateFastBlinkContinues, "");
				}
				tv.tv_sec = 1 << ((ses->retransmits)-1);
			}
			fprintf(stderr,"req_name %s, retransmits = %d , tv.tv_sec = %d\n", req_name, ses->retransmits,  (int)tv.tv_sec);
			fflush(stderr);
		}
		else{
			if(startNetDiscovery())
			{  /*RG Activation*/
	    			tv.tv_sec = 3;
			}
			else
			{	/*RG Regular Operations*/			
				if(ses->retransmits > PPP_MAX_RETRY_INDEX)
				{
					ses->retransmits = PPP_REPEAT_RETRY_STARTINDEX;
				}
				if(ses->retransmits > 1)
				{
					devCtl_boardIoctl(BOARD_IOCTL_LED_CTRL, 0, "",  kLedInfo, kLedFailStateFastBlinkContinues, "");
				}
				tv.tv_sec = 1 << ((ses->retransmits)-1);
			}		
			fprintf(stderr,"req_name %s, retransmits = %d , tv.tv_sec = %d\n", req_name, ses->retransmits,  (int)tv.tv_sec);
	    ...
	    if (tv.tv_sec > 3)
		tv.tv_sec = 3;
	#endif /*__MSTC__, chris 2012-11-19*/
	    tv.tv_usec = 0;
	...
	if( ret == 0 ){
		...
		if( ret != 0 )
	       ...
	       #if 1 /*__MSTC__, chinglung 2014-03-11*/
	...
	case PADO_CODE:		/* wait for PADO */
	{
		...
		if( ret != 0){
			...
			#if 1 /*__MSTC__, chris*/
			if(!strcmp(req_name, (char *) PPP3IFNAME)  || !strcmp(req_name, (char *) PPP4IFNAME)){
			if(startNetDiscovery()){
				DiscoverChange = FALSE;
				if(rcv_packet.tags[TAG_AC_NAME]->tag_data){	
					identifyServiceMode(rcv_packet.tags[TAG_AC_NAME]->tag_data);
					break;
				}
			}else{
				cmsLog_notice("normal pppoed session\n");
			}
			}else if(!strcmp(req_name, (char *) PPP1IFNAME))
			{
				if(startNetDiscovery()){
					ppp1DiscoverFail = FALSE;
				}
			}
			else if(!strcmp(req_name, (char *) PPP2IFNAME))
			{
				if(getActivatedStatus(ACTIVATION_IS_BBI_ACTIVATED) == 0)
				{
					/*BBI is not activated, and the server does not send the PADO*/
					ppp2DiscoverFail = FALSE;
				}
			}
			#endif
	    }
	...
	case PADS_CODE:
	{
		...
		if( ret != 0){
		#if 1 /*__MSTC__, chinglung*/
		if(rcv_packet.tags[TAG_SYS_ERR])
		{	
			systemErr = TRUE;
		}
		else
	...
	#if 1 /*__MSTC__, chris 2012-11-19, wait 3 seconds for lastest PADI */
	if(!strcmp(req_name, (char *) PPP3IFNAME)  || !strcmp(req_name, (char *) PPP4IFNAME)){
	if(DiscoverChange && startNetDiscovery()){	
		UBOOL8 bDscChng;		
		bDscChng = TRUE;
		sendPPPoEDiscoverEventMessage(bDscChng, NULL);		
	}
	else
		{
			fprintf(stderr, " -- no PVC change or  startNetDiscovery fail\n");
		}
	}else if(!strcmp(req_name, (char *) PPP1IFNAME))
	{
		if((ppp1DiscoverFail ||systemErr) && startNetDiscovery())
		{
			int retryNum=0;
			int pinChecked=0;
			setActivatedStatus(ACTIVATION_ISPPPRETRY, TRUE_STR);
			retryNum = getActivatedStatus(ACTIVATION_PPPRETRYNUM);
			
			pinChecked = getActivatedStatus(ACTIVATION_PINCHECKED);
			
			persist=0;
			if(retryNum< MAXPPPRETRY)
			{

				if(pinChecked){
				char buf[sizeof(CmsMsgHeader) + sizeof(PppoeDiscoverMsgBody)]={0};
				CmsMsgHeader *msg=(CmsMsgHeader *) buf;
				PppoeDiscoverMsgBody *pppoeDscBody = (PppoeDiscoverMsgBody *) (msg+1);
				CmsRet ret;
				cmsLog_error(" send PPP Retry Event Message\n");
						
				msg->type = CMS_MSG_PPP_RETRY; 				
				msg->src = EID_PPP; //MAKE_SPECIFIC_EID(getpid(), EID_PPP);
				msg->dst = EID_NETDISCOVERY;
				msg->flags_event = 0;
				msg->dataLength = sizeof(PppoeDiscoverMsgBody);
				sprintf(pppoeDscBody->ifname, "%s", req_name);
					
				if ((ret = cmsMsg_send(msgHandle, msg)) != CMSRET_SUCCESS)
				{
				  cmsLog_debug("Could not send out CMS_MSG_PPP_RETRY, ret=%d", ret);
				}
				else
				{
				  cmsLog_debug("Sent out CMS_MSG_PPP_RETRY, ret=%d", ret);
				}   				
				}
				else{
					persist=1; 
					retryNum++;
					setActivatedStatus(ACTIVATION_PPPRETRYNUM, retryNum);
					setActivatedStatus(ACTIVATION_ISPPPRETRY, TRUE_STR);
					
					setActivatedStatus(ACTIVATED_STATUS_STATUS, ACTIVATED_STATUS_004);
					setActivatedInfo(ACTIVATION_RETRYIFNAME, PPP1IFNAME);					
				}
			}
			else{
				devCtl_boardIoctl(BOARD_IOCTL_LED_CTRL, 0, "",  kLedInfo, kLedStateFail, ""); /*For info Led, ChingLung*/

				/*retry number had 5 times*/			
				setActivatedStatus(ACTIVATED_STATUS_STATUS, ACTIVATED_STATUS_007);	
				setActivatedInfo(ACTIVATION_SERVICEMODE, SMODE_UNKNOWN);
			}
		}
		else if(ppp1DiscoverFail || systemErr)
		{
			cmsLog_error("ppp0.1 is not esbalished, because no client slots available or no PADO, and keep retry");
			UINT32 retryNum = 0;
			retryNum = getActivatedStatus(PPP1_AUTH_FAIL_RETRY_INDEX);
			retryNum++;
			if(retryNum > PPP_MAX_RETRY_INDEX)
			{
				retryNum = PPP_REPEAT_RETRY_STARTINDEX;
			}
			setActivatedStatus(PPP1_AUTH_FAIL_RETRY_INDEX,retryNum);
			if(systemErr)
				devCtl_boardIoctl(BOARD_IOCTL_LED_CTRL, 0, "",  kLedInfo, kLedFailStateFastBlinkContinues, ""); /*For info Led, ChingLung*/
		}
	}
	else if(!strcmp(req_name, (char *) PPP2IFNAME))
	{
		if((ppp2DiscoverFail ||systemErr)  && getActivatedStatus(ACTIVATION_IS_BBI_ACTIVATED) == 0)
		{	
			persist=0; 
			cmsLog_error("BBI does not retry");
		}
		else if(ppp2DiscoverFail ||systemErr)
		{
			cmsLog_error("ppp0.2 is not esbalished, because no client slots available or no PADO, and keep retry");
			UINT32 retryNum = 0;
			retryNum = getActivatedStatus(PPP2_AUTH_FAIL_RETRY_INDEX);
			retryNum++;
			if(retryNum > PPP_MAX_RETRY_INDEX)
			{
				retryNum = PPP_REPEAT_RETRY_STARTINDEX;
			}
			setActivatedStatus(PPP2_AUTH_FAIL_RETRY_INDEX,retryNum);
		}
	}
	#endif /*__MSTC__, chris 2012-11-19*/
...
#if 1 /*__MSTC__, chris 2012-11-02*/
int identifyServiceMode(char *ac_name){

	UBOOL8 bDscChng=FALSE;
	char L2Info[BUFLEN_64]={'\0'};
	getActivatedInfo(ACTIVATION_L2_DISCOVER_STATUS, L2Info);
	
	if(cmsUtl_strstr(L2Info,ADSL_DISCOVERY)){
		if(cmsUtl_strstr(req_name, PPP3IFNAME)){					
					if(cmsUtl_strstr(ac_name,AC_HANSENET_XDSL)){
						sendPPPoEDiscoverEventMessage(bDscChng, ADSL_SMODE_HANSENETULL);
					}else if(cmsUtl_strstr(ac_name,AC_TK_ENABLED)){
						sendPPPoEDiscoverEventMessage(bDscChng,ADSL_SMODE_WHOLESALETK);
					}else {
						sendPPPoEDiscoverEventMessage(bDscChng,ADSL_SMODE_WHOLESALEPIN);
					}				
		}else if(cmsUtl_strstr(req_name, PPP4IFNAME)){
					sendPPPoEDiscoverEventMessage(bDscChng,ADSL_SMODE_BITSTREAM);
				}
		else
		{
			cmsLog_error("req_name is not ppp0.3 and ppp0.4, req_name=%s", req_name);
		}
	}else if(cmsUtl_strstr(L2Info,ETH_DISCOVERY)){				
		if(cmsUtl_strstr(req_name, PPP3IFNAME)){
					if(cmsUtl_strstr(ac_name,AC_HANSENET_XDSL)){
						sendPPPoEDiscoverEventMessage(bDscChng,ETH_SMODE_ETHER);
					}else if(cmsUtl_strstr(ac_name,AC_TK_ENABLED)){
						sendPPPoEDiscoverEventMessage(bDscChng,ETH_SMODE_WHOLESALETK);
					}else {
						sendPPPoEDiscoverEventMessage(bDscChng,ETH_SMODE_WHOLESALEPIN);
					}											
		}else if(cmsUtl_strstr(req_name, PPP4IFNAME)){
					sendPPPoEDiscoverEventMessage(bDscChng,ETH_SMODE_BITSTREAM);	
					}
		else
		{
			cmsLog_error("req_name is not ppp0.3 and ppp0.4, req_name=%s", req_name);
		}
	}else if(cmsUtl_strstr(L2Info,VDSL_DISCOVERY)){
		if(cmsUtl_strstr(req_name, PPP3IFNAME)){
					if(cmsUtl_strstr(ac_name,AC_HANSENET_XDSL)){
						sendPPPoEDiscoverEventMessage(bDscChng,VDSL_SMODE_HANSENETULL);
					}else if(cmsUtl_strstr(ac_name,AC_TK_ENABLED)){
						sendPPPoEDiscoverEventMessage(bDscChng,VDSL_SMODE_WHOLESALETK);
					}else {
						sendPPPoEDiscoverEventMessage(bDscChng,VDSL_SMODE_WHOLESALEPIN);
					}						
		}else if(cmsUtl_strstr(req_name, PPP4IFNAME)){
					sendPPPoEDiscoverEventMessage(bDscChng,VDSL_SMODE_BITSTREAM);
				}
		else
		{
			cmsLog_error("req_name is not ppp0.3 and ppp0.4, req_name=%s", req_name);
	...
	#if 0 /* __MSTC__, chris 2012-11-05 for web redirect*/
	if (strstr(ac_name, "HANSENET-XDSL")){											
		if ((ret = cmsLck_acquireLockWithTimeout(CLI_LOCK_TIMEOUT)) != CMSRET_SUCCESS)
   		{
			cmsLog_error("failed to get lock, ret=%d", ret);
			return 0;
		}
		cmsDal_setRedirectIptables(TRUE);				
		cmsLck_releaseLock();
	}	
	#endif
	return 1;
...
/** 
 * Send pvc change event message to smd
 *
 */
void sendPPPoEDiscoverEventMessage(UBOOL8 bDscChng, char *serviceMode)
{
	char buf[sizeof(CmsMsgHeader) + sizeof(PppoeDiscoverMsgBody)]={0};
	CmsMsgHeader *msg=(CmsMsgHeader *) buf;
	PppoeDiscoverMsgBody *pppoeDscBody = (PppoeDiscoverMsgBody *) (msg+1);
	CmsRet ret;
	cmsLog_error(" sendPPPoEDiscoverEventMessage\n");
			
	if(bDscChng){
		msg->type = CMS_MSG_PPPOE_DISCOVER_CHANGED; 
		pppoeDscBody->DscChange = TRUE;
		sprintf(pppoeDscBody->serviceMode,"%s", "Failed");
	}else{
		msg->type = CMS_MSG_PPPOE_DISCOVER_SUCCESS; 
		pppoeDscBody->rcvPADO = TRUE;
		sprintf(pppoeDscBody->serviceMode,"%s", serviceMode);
	}
	msg->src = EID_PPP; //MAKE_SPECIFIC_EID(getpid(), EID_PPP);
	msg->dst = EID_NETDISCOVERY;
	msg->flags_event = 0;
   ...
   if ((ret = cmsMsg_send(msgHandle, msg)) != CMSRET_SUCCESS)
   {
      cmsLog_debug("Could not send out CMS_MSG_PPP_PVC_CHANGED, ret=%d", ret);
   }
   else
   {	
      cmsLog_debug("Sent out CMS_MSG_PPP_PVC_CHANGED, ret=%d", ret);
   }   
}

UBOOL8 startNetDiscovery(){
	UBOOL8 startNetDsc=FALSE;
	int sActivated = 0;
	sActivated = getActivatedStatus(ACTIVATION_ACTIVATED);

	if(sActivated == FALSE_STR)
	{
		startNetDsc = TRUE;
	}
	return startNetDsc;
