...
#include <syscall.h>
#endif
#ifdef MSTC_TELEFONICA_CUSTOMIZATION /* Follow 11n model to support pppoe log. __TELEFONICA__, MSTC Stan Su, 20120220. */
#include "oal.h"
#endif
...
#ifdef MSTC_TELEFONICA_CUSTOMIZATION /* Follow 11n model to support pppoe log. __TELEFONICA__, MSTC Stan Su, 20120220. */
    oalLog_syslog(LOG_LEVEL_NOTICE, "PADO received");
#endif
    
#if 1 /*__MSTC__, chris 2013-01-04*/

	int activatedStage = 0;
	int activated = 0;
	activated = getActivatedStatus(ACTIVATION_ACTIVATED);
	activatedStage = getActivatedStatus(ACTIVATED_STATUS_STAGE);
if( activated ==TRUE_STR ||  activatedStage == PPP_ACTIVATION_SESS_STAGE || !strcmp(req_name, PPP2IFNAME))
{
#endif  
...
#ifdef MSTC_TELEFONICA_CUSTOMIZATION /* Follow 11n model to support pppoe log. __TELEFONICA__, MSTC Stan Su, 20120220. */
    oalLog_syslog(LOG_LEVEL_NOTICE, "Send PADR");
#endif
    (*p_out) = &ses->curr_pkt;
...
#ifdef MSTC_TELEFONICA_CUSTOMIZATION /* Follow 11n model to support pppoe log. __TELEFONICA__, MSTC Stan Su, 20120220. */
    oalLog_syslog(LOG_LEVEL_NOTICE, "Sending PADI");
#endif
    if (DEB_DISC)
...
#ifdef MSTC_TELEFONICA_CUSTOMIZATION /* Follow 11n model to support pppoe log. __TELEFONICA__, MSTC Stan Su, 20120220. */
    oalLog_syslog(LOG_LEVEL_NOTICE, "Receive PADS");
#endif
...
#ifdef MSTC_TELEFONICA_CUSTOMIZATION /* Follow 11n model to support pppoe log. __TELEFONICA__, MSTC Stan Su, 20120220. */
    oalLog_syslog(LOG_LEVEL_NOTICE, "Receive PADT");
#endif
...
//    poe_info (ses,"init_ses: creating socket");
	...
	#if 1 /*__MSTC__, chris 2013-01-24*/
	/* RG regular operations, don't set session retry limit */

	int activated = 0;
	int isBBIActivated = 0;
	activated = getActivatedStatus(ACTIVATION_ACTIVATED);
	isBBIActivated = getActivatedStatus(ACTIVATION_IS_BBI_ACTIVATED);
	if( activated ==TRUE && !strcmp(req_name, PPP1IFNAME))
	{
		ses_retries = -1;
	}
	else if(isBBIActivated==TRUE && !strcmp(req_name, PPP2IFNAME))
	{
		ses_retries = -1;
	}
	#endif /*__MSTC__, chris 2013-01-24*/
