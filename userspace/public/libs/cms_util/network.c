...
#if 1 
/* WenJuin@MSTC: check address is form LAN or not, 20130414*/
      if (!strncasecmp(ipAddr, "FE80:",5))
      {
         cmsLog_notice("%s is lanside",ipAddr);      
         return onLanSide = TRUE;
      }
#endif
...
CmsRet cmsSpellRaAddress(char* sitePrefix,char* localaddr,char *ip_addr)
{
    UINT8 subnetId = ~0;
    struct in6_addr in6Addr = IN6ADDR_LOOPBACK_INIT;
    char prefix_addr[BUFLEN_48];
    struct in6_addr local_addr = IN6ADDR_LOOPBACK_INIT;
    CmsRet ret;
    int prefix_length = 0;

    memset(prefix_addr,0,sizeof(prefix_addr));
    prefix_length = cmsNet_getPrefixLength(sitePrefix);
    /* subnetID should be 0xff >> (8 - (64 - prefix)) */
    /* Prefix: 56 subnetId: 0xff, Prefix: 57 subnetId: 0x7f ..., Prefix: 64 subnetId: 0x00 */
    subnetId >>= prefix_length - 56;

    if ((ret = cmsNet_subnetIp6SitePrefix(sitePrefix, subnetId, 64, prefix_addr)) != CMSRET_SUCCESS)
    {
        cmsLog_error("prefix extract failed!!");
        return CMSRET_INVALID_ARGUMENTS;
    }

    if (inet_pton(AF_INET6, prefix_addr, &in6Addr) <= 0)
    {
        cmsLog_error("Invalid sitePrefix=%s", prefix_addr);
        return CMSRET_INVALID_ARGUMENTS;
    }
    if (inet_pton(AF_INET6, localaddr, &local_addr) <= 0)
    {
        cmsLog_error("Invalid Link local address=%s", localaddr);
        return CMSRET_INVALID_ARGUMENTS;
    }


    sprintf(ip_addr, "%x:%x:%x:%x:%x:%x:%x:%x",
            in6Addr.s6_addr16[0], in6Addr.s6_addr16[1], in6Addr.s6_addr16[2], in6Addr.s6_addr16[3],
            local_addr.s6_addr16[4], local_addr.s6_addr16[5],local_addr.s6_addr16[6],local_addr.s6_addr16[7]);
    return CMSRET_SUCCESS;
}
#endif
