...
#if 1 /*For To2 random seed, Mitrastar ChingLung, 20130215*/
#include "board.h"
...
/* CPE sends TCP FIN cause TR069 session interrupt. __TELEFONICA__ MSTC Stan Su 20120413.*/
...
CmsRet cmsTmr_resetTimeByName(void *handle, UINT32 ms, const char *name)
{
   ...
   if (found)
   {
      ...
      func = tmrEvent->func;
      cmsTmr_cancel((void*)tmrHandle, func, (void*)tmrEvent->ctxData);
      return(cmsTmr_set(handle, func, ctxData, ms, name));   
   } /* found */
...
#if 1 /*For To2 random seed, Mitrastar ChingLung, 20130215*/
UINT32 cmsGetRandomNumber(time_t t)
{
	UINT32 randomNumber=0;
	NVRAM_DATA nvramData;
	char serialNumberStr[BUFLEN_32];
	UINT32 serialNumber=0;
	char macStr[BUFLEN_32];
	UINT32 mac=0;
	UINT32 seed=0;
	CmsTimestamp currentTime;
	sysGetNVRAMFromFlash( &nvramData );
	
	sprintf(serialNumberStr, "%s", nvramData.SerialNumber);
	serialNumber = atol(serialNumberStr+4);

	sprintf(macStr, "%02X%02X%02X%02X%02X%02X", *((UINT8 *)&nvramData.ucaBaseMacAddr[0])
												, *((UINT8 *)&nvramData.ucaBaseMacAddr[1])
												, *((UINT8 *)&nvramData.ucaBaseMacAddr[2])
												, *((UINT8 *)&nvramData.ucaBaseMacAddr[3])
												, *((UINT8 *)&nvramData.ucaBaseMacAddr[4])
												, *((UINT8 *)&nvramData.ucaBaseMacAddr[5]));
	mac = atol(macStr+3);
	cmsTms_get(&currentTime);
	seed = currentTime.nsec<<16 | serialNumber<<8 | mac;
	seed = seed * 1103515245+12345;
	cmsLog_debug("current time=%d", currentTime.nsec);
	cmsLog_debug("seed=%d", seed);
	srand(seed);
	randomNumber = rand() & 0x7FFFFFFF;
	cmsLog_debug("randomNumber=%d",randomNumber);
	return randomNumber;
}
#endif
