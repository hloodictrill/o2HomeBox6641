...
#if 1
/* WenJuin@MSTC: single session user, 20130509 */
void cmsTms_getSysuptime(CmsTimestamp *tms)
{
	...
	fscanf(fd,"%d.%d",&tms->sec,&tms->nsec);
	fclose(fd);
}
#endif /* 1 */
...
#if 1
/*For To2 display wan up time, Mitrastar ChingLung, 20130604*/
CmsRet cmsTms_getHoursMinutesSeconds(UINT32 t, char *buf, UINT32 bufLen)
{
    ...
    memset(buf, 0, bufLen);
    r = snprintf(buf, bufLen-1, "%02d:%02d:%02d", hours, minutes, seconds);
    if (r >= bufLen)
    {
        ret = CMSRET_RESOURCE_EXCEEDED;
    }
...
CmsRet cmsTms_getDaysHoursMinutesSeconds_TO2(UINT32 t, char *buf, UINT32 bufLen)
{
    UINT32 days, hours, minutes, seconds, month, year;
    SINT32 r;
    CmsRet ret=CMSRET_SUCCESS;

    year = t / (SECS_IN_DAY * 30 * 12);
    t -= (year * (SECS_IN_DAY * 30 * 12));

    month = t / (SECS_IN_DAY * 30);
    t -= (month * (SECS_IN_DAY * 30));
    ...
    memset(buf, 0, bufLen);
    r = snprintf(buf, bufLen-1, "%d, %d, %d, %d, %d, %d", year, month, days, hours, minutes, seconds);
    if (r >= bufLen)
    {
        ret = CMSRET_RESOURCE_EXCEEDED;
    }
