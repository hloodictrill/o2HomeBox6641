...
CmsRet cmsUtl_atmVpiVciStrToNum(const char *vpiVciStr, SINT32 *vpi, SINT32 *vci)
{
   ...
   *vpi = *vci = -1;   
...
#if 1 /*MSTC, chris 2013-01-14*/
   if ((prefix = cmsUtl_strstr(vpiStr, DSL_LINK_DESTINATION_PREFIX_PVC)) == NULL)
   {
   ...
   {
/* __TELEFONICA__, ZyXEL Eva, 20100608. */
/* Do nothing (PTM mode does not use PVC as prefix and for now we only have PTM) */ 	
#ifdef MTS_Telefonica_TR69	
	  cmsLog_debug("Do nothing (PTM mode does not use PVC as prefix) \n");
...
UBOOL8 cmsUtl_isValidIpv4Address(const char* input)
{
   ...
   /* IP address has the following format
      xxx.xxx.xxx.xxx where x is decimal number */
   pToken = strtok_r(buf, ".", &pLast);
/* Fix uncaught signal number error, __TELEFONICA__, MitraStra Chehuai, 20100610 */
#ifdef MTS_TELEFONICA_FIREWALL 
...
/* Jessica@MSTC: Check case. ex. "1.2.3.4.5.6" */
      pToken = strtok_r(NULL, ".", &pLast);
      if (!IS_EMPTY_STRING(pToken))
	  return FALSE;
...
/*KeyYang@MSTC: Merge from Telefonica_Common, 20120712*/
#ifdef MSTC_COMMON_PATCH
/* Check the port range, e.g. "ExternalPortEndRange" must not smaller than "ExternalPort", __TELEFONICA__, ZyXEL Eva, 20091216.  */
#ifdef MTS_Telefonica_TR69  
UBOOL8 cmsUtl_isValidPortRange(const UINT32 startPort, const UINT32 endPort)
{
   UBOOL8 ret = TRUE;
   
	/* The parameter endPort shouled be able to accept the value 0, which means that only 
	 *  one port is configured, __TELEFONICA__, ZyXEL Eva, 20100608.  */
...
/* Check the validity of IP addr and mask, __TELEFONICA__, MitraStar Chehuai, 20110613.  */
#ifdef MTS_TELEFONICA_FIREWALL 
UBOOL8 cmsUtl_isIPAddr(const char* input)
{
	char addr[128];
	int i = 0;
	int isIPAddress = TRUE; 
...
/*KeyYang@MSTC: Merge from Telefonica_Common, 20120712*/
      ...
      /*__MTS__, TengChang, MTS LOG parameters, migrate from Common_406*/
      if ( src == NULL && dest != NULL ) {
         strcpy(dest, "");
      }
