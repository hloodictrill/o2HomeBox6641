...

/* MSTC , update Conntrack when receive rip info, by Andylin */
#ifdef MSTC_RIP2UPDATE_CONNTRACK
void
MSTC_Rip_updateConntrack(struct route_node *rn)
{
      char cmd[64], ripPrefix[32], fbuf[256], buf[32], pf;
	  char *ptr = NULL, *ptr2 = NULL, *ptr3 = NULL, *ptr4 = NULL;
      char prefixStr[32];      

      unsigned char *p = &rn->p.u.prefix;

	  int plen1 = rn->p.prefixlen / 8;
  	  int plen2 = ((rn->p.prefixlen % 8)==0)?0:(8-(rn->p.prefixlen % 8));
	  
	  switch(plen1)
	  {
		case 1:
			snprintf(ripPrefix, sizeof(ripPrefix), "%d.", p[0]);
			pf = p[1];
			break;
		case 2:
			snprintf(ripPrefix, sizeof(ripPrefix), "%d.%d.", p[0], p[1]);
			pf = p[2];
			break;
		case 3:
			snprintf(ripPrefix, sizeof(ripPrefix), "%d.%d.%d.", p[0], p[1], p[2]);
			pf = p[3];
			break;
		case 4:
			snprintf(ripPrefix, sizeof(ripPrefix), "%d.%d.%d.%d", p[0], p[1], p[2], p[3]);
			pf = 0;
			break;
		default:
			break;
      ...
      snprintf( cmd, sizeof(cmd), "grep %s /proc/net/nf_conntrack > /tmp/Conntrack.tmp", ripPrefix);
      system(cmd);

      FILE* fs = NULL;
      int count;
      if ((fs = fopen("/tmp/Conntrack.tmp", "r")) != NULL) 
      {
          while (!feof(fs)) 
	      {		
		     fgets(fbuf, sizeof(fbuf), fs);			 
			 for(count=0; count<(1<<plen2); count++)
			 {
			    char tmp2[32];
				memset(prefixStr, 0, sizeof(prefixStr));
				strncpy(prefixStr, ripPrefix, sizeof(prefixStr));
				if(plen2 != 0)
				{
				    char tmp[5];
				    snprintf(tmp, sizeof(tmp), "%d.", pf|count);					
					strcat(prefixStr, tmp);
				}
		        //printf("!!!!!!!!! %s : %s \n", inet_ntop(rn->p.family, &rn->p.u.prefix, tmp2, sizeof(tmp2)), prefixStr);
			 
			 	snprintf( buf, sizeof(buf), "dst=%s", prefixStr);
				if(buf)
				 	ptr = strstr(fbuf, buf);	
				if(ptr)
					ptr3 = strtok_r(ptr, " " ,&ptr2);
				if(ptr3)
				 	ptr4 = strstr(ptr3, prefixStr);
				if(ptr4){
				  	memset(cmd, 0, sizeof(cmd));		
				  	snprintf( cmd, sizeof(cmd), "conntrack -D -d %s > /dev/null", ptr4);
					//printf("!!!!!!!! %s\n", cmd);
				  	system(cmd);	      
				}
			 }
...
/* MSTC , update Conntrack when receive rip info, by Andylin */
#ifdef MSTC_RIP2UPDATE_CONNTRACK	  
      if(select->type == ZEBRA_ROUTE_RIP){
	     MSTC_Rip_updateConntrack(rn);
      }
#endif			  
