...
#include <resolv.h>
#include "libbb.h"
#ifdef MSTC_COMMON_PATCH /* __MTS__, Ailsa, Support nslookup, 20110607*/
#include <stdint.h>
#include <netdb.h>
...
DNSResult *r;
extern int h_errno;
#endif /* __MTS__, Ailsa, Support nslookup*/
...
static int print_host(const char *hostname, const char *header)
{
	...
	if (!rc) {
		...
		unsigned cnt = 0;
		#ifdef MSTC_COMMON_PATCH//__MTS__,Ailsa,nslookup,20110607
...
int nslookup_main(int argc, char **argv)
{
	 ...
	 * standard nslookup */
	#if 0 //__MTS__,Ailsa,nslookup,20110607
	...
	while ((c = getopt(argc, argv, "s:h:f:")) != -1) {
		switch(c) {
			case 's':
