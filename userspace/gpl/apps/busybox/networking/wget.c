...
#ifdef MSTC_COMMON_PATCH //__MSTC__, Justin, Supoprt Tr143: DownloadDiagnostic
#include <time.h>
#include "cms_msg.h"
#include "cms_util.h"
#include "cms_log.h"
#endif
#ifdef MTS_Telefonica_TR69	/* For Download diagnostic in TR143, __TELEFONICA__, ZyXEL Eva, 20091015.  */
...
#endif
#ifdef MTS_Telefonica_TR69	/* Support parameter "TotalBytesReceived" for Download diagnostic in TR143, __TELEFONICA__, Mitrastar Eva, 20110324.  */
...
static void sendEventMessage(CmsMsgType msgType)
{
	...
	{
		cmsLog_error("Send out 0x%x event msg.", msgType);
	}
	return;
}
#endif
...
#ifdef MTS_Telefonica_TR69	/* Support parameter "TotalBytesReceived" for Download diagnostic in TR143, __TELEFONICA__, Mitrastar Eva, 20110324.  */
void catchIfStatistics(char *ifName, unsigned long *bytes)
{
	FILE *fp;
	int read;
	int receiveBytes;
...
enum {
...
#ifdef SUPPORT_TELEFONICA /* Modify TR143, __TELEFONICA__, Mitrastar Jennifer, 20111205 */
    ...
    WGET_OPT_SESSIONNUM = (1 << 12),
    WGET_OPT_INTERFACE  = (1 << 13),
    WGET_OPT_WRITEIGNORE= (1 << 14),
    WGET_OPT_DSCP       = (1 << 15),
    WGET_OPT_ETNERPRIORITY = (1 << 16),
#endif
#endif
};
...
#ifdef MTS_Telefonica_TR69	/* Support parameter "TotalBytesReceived" for Download diagnostic in TR143, __TELEFONICA__, Mitrastar Eva, 20110324.  */
#ifndef SUPPORT_TELEFONICA/* Modify TR143, __TELEFONICA__, Mitrastar Jennifer, 20111205 */
		if(interfaceCatch)
		{
...
#endif
		testBytesReceived = G.transferred;
...
int wget_main(int argc UNUSED_PARAM, char **argv)
{
...
#endif
#ifdef MTS_Telefonica_TR69	/* Support parameter "TotalBytesReceived" for Download diagnostic in TR143, __TELEFONICA__, Mitrastar Eva, 20110324.  */
#if SUPPORT_TELEFONICA /* Modify TR143, __TELEFONICA__, Mitrastar Jennifer, 20111205 */
    char *sessionNumber;
    char *requestEid;
#endif
	char *ifName;
	int interfaceCatch=0;
#endif
#ifdef MTS_Telefonica_TR69	/* DSCP & EthernetPriority for upload diagnostic profile in TR143, __TELEFONICA__, ZyXEL Eva, 20100112.  */
	char *strVal1 = 0, *strVal2 = 0;
	int dscpMark = 0;
...
#ifdef MTS_Telefonica_TR69
        "sessionNumber"      No_argument       "s"
        "interfaceName"      No_argument       "i"
        "writeIgnore"        No_argument       "w"
        "DSCP"               No_argument       "d"
        "ethernetPriority"   No_argument       "e"
...
#ifdef MTS_Telefonica_TR69
#ifdef SUPPORT_TELEFONICA /* Modify TR143, __TELEFONICA__, Mitrastar Jennifer, 20111205 */
	opt = getopt32(argv, "csqO:P:Y:U:n:i:wd:e:R:",
			&fname_out,  &dir_prefix, &proxy_flag, &user_agent, &sessionNumber, &ifName,  &strVal1, &strVal2, &requestEid
			IF_FEATURE_WGET_LONG_OPTIONS(, &headers_llist)
			IF_FEATURE_WGET_LONG_OPTIONS(, &post_data)
			);
#else
	opt = getopt32(argv, "csqO:P:Y:U:d:e:i:" /*ignored:*/ "t:T:",
				&fname_out, &dir_prefix,
				&proxy_flag, &user_agent,
                &strVal1, &strVal2, &ifName,
				NULL, /* -t RETRIES */
...
#endif
	dscpMark = atoi(strVal1);
	priorityMark = atoi(strVal2);
#else
 ...
 resolve_lsa:
	...
	if (quiet_flag==FALSE) {
	  ...
	  if ( server.host != NULL && *(server.host) != '\0')
	  {
	  	 char cmd[1024];
	  	 ...
	  	 {
			cmd[0] = '\0';
			sprintf(cmd, "iptables -t mangle -I OUTPUT -j DSCP --set-dscp 0x%x -d %s >/dev/null &",
					dscpMark, inet_ntoa(s_in.sin_addr));
			system(cmd);
	  	 }

		 if ( priorityMark != 0x1000 )  /* if (priority != 0)   //priorityMark = (priority << 13) | 0x1000 */ 
	  	 {
		 	cmd[0] = '\0';
			sprintf(cmd, "iptables -t mangle -I OUTPUT -j MARK --set-mark 0x%x -d %s >/dev/null &",
					priorityMark, inet_ntoa(s_in.sin_addr));
			system(cmd);
		 }

		 cmd[0] = '\0';
 		 sprintf(cmd, "iptables -t mangle -I POSTROUTING -j ACCEPT -d %s >/dev/null &", inet_ntoa(s_in.sin_addr));
		 system(cmd);
	  }
#endif	
...
#ifdef WGET_DEBUG	/* __TELEFONICA__, ZyXEL Eva, 20091015.  */
...
*/
		...
		case 206:
			...
			/* fall through */
		default:
#ifdef SUPPORT_TELEFONICA /* Download diagnostic failed, update diagnosticsState, __TELEFONICA__, MSTC Stan Su, 20120213. */
			strcpy(diagnosticsState, "Error_InitConnectionFailed");
			createTR143Info();
#endif
...
//			bb_error_msg_and_die("bad redirection (no Location: header from server)");
...
#if defined (SUPPORT_TELEFONICA) || defined (MTS_Telefonica_TR69) /* Modify TR143, __TELEFONICA__, Mitrastar Jennifer, 20111205 */
	...
	if (server.host != NULL && *(server.host) != '\0')
	{
		...
		{
        	cmd[0] = '\0';
			sprintf(cmd, "iptables -t mangle -D OUTPUT -j DSCP --set-dscp 0x%x -d %s >/dev/null &",
					dscpMark, inet_ntoa(s_in.sin_addr));
			system(cmd);
		}
		...
		cmd[0] = '\0';
 		sprintf(cmd, "iptables -t mangle -D POSTROUTING -j ACCEPT -d %s >/dev/null &", inet_ntoa(s_in.sin_addr));
		system(cmd);
	}
#endif	
