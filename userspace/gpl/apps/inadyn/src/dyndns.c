...
#include <time.h>
#if 1
#include <net/if.h>			/* For get wan interface ip address */
#include <sys/ioctl.h>
#include "oal.h"				/* For msgHandle */
#include "cms_util.h"			/* For CMSMEM_REPLACE_STRING function */
#include "activated.h"
#include "cms_log.h"			/* For cmsLog_debug & cmsLog_error */
extern void *msgHandle; 		/* handle to communications link to smd */
#endif  /*1*/
...
#if 1
static int get_req_for_changeip_http_dns_server(DYN_DNS_CLIENT *p_self, int cnt,  DYNDNS_SYSTEM *p_sys_info);
static int get_req_for_dnsdynamic_http_dns_server(DYN_DNS_CLIENT *p_self, int cnt,  DYNDNS_SYSTEM *p_sys_info);
#endif  /*1*/
...
/*[IreneWang@MSTC:TO2]$ Support DDNS, 20130227*/
    ...
    {DNSDYNAMIC_DEFAULT, 
        {"default@dnsdynamic.org", NULL,  
            (DNS_SYSTEM_SRV_RESPONSE_OK_FUNC)is_dyndns_server_rsp_ok, 
            (DNS_SYSTEM_REQUEST_FUNC) get_req_for_dnsdynamic_http_dns_server,
            DYNDNS_MY_IP_SERVER, "/",
			"www.dnsdynamic.org", "/api/?", ""}},		
#endif  /*1*/
...
{
	(void)p_sys_info;
	return sprintf(p_self->p_req_buffer, GENERIC_DNSDYNAMIC_AUTH_MY_IP_REQUEST_FORMAT,
...
/*[IreneWang@MSTC:TO2]$ Support DDNS, 20130227*/
#if 1
							else if (strstr(p_self->info.dyndns_server_name.name, CHANGE_IP_SYSTEM_DYNAMIC) != NULL)
							{
								if (strstr(http_tr.p_rsp, DYNDNS_OK_RESPONSE) != NULL)
								{
									fprintf(fp,"/is_success=%d",1); /*updated on server is success.*/
							...
							else if (strstr(p_self->info.dyndns_server_name.name, DNSDYNAMIC_IP_SYSTEM_DYNAMIC) != NULL)
							{
								if (strstr(http_tr.p_rsp, DYNDNS_OK_RESPONSE) != NULL)
								{
									fprintf(fp,"/is_success=%d",1); /*updated on server is success.*/
...
/*[IreneWang@MSTC:TO2]$ DDNS update mechanism from polling to event driven and update RG wan IP Address, 20130917*/
		/* Get RG wan ip address */
		int fd = 0;
		struct ifreq ifr;
		FILE *fPtr = NULL;
		int dual = 0;
		char ipAddress[IP_V4_MAX_LENGTH] = {'\0'};
		...
		if (fPtr) {
			fscanf(fPtr, "%d", &dual);
			if (dual) {
				strncpy(ifr.ifr_name, PPP2IFNAME, IFNAMSIZ-1);
			}
			else {
				strncpy(ifr.ifr_name, PPP1IFNAME, IFNAMSIZ-1);
			}
			fclose(fPtr);
		}
		else {
			cmsLog_error("Fail to open file when get wan ip address");
			rc = RC_OS_INVALID_IP_ADDRESS;
			return rc;
		}

		if (ioctl(fd, SIOCGIFADDR, &ifr) < 0)	/* If return error after ioctl, treat as wan not ready */
		...
		CMSMEM_REPLACE_STRING(p_self->http_tr.p_rsp, ipAddress);
		p_self->http_tr.rsp_len = strlen(inet_ntoa(((struct sockaddr_in *)&ifr.ifr_addr)->sin_addr));

		if (p_self->http_tr.p_rsp)
			cmsLog_debug("My WAN IP Address is %s, length is %d", p_self->http_tr.p_rsp, p_self->http_tr.rsp_len);
#else
...
int dyn_dns_main(DYN_DNS_CLIENT *p_dyndns, int argc, char* argv[])
{
	...
	do
	{
		...
		while(1)
		{
			...
			cmsLog_debug("The result for update up is %d", rc);
			if (rc != RC_OK)
			{
...
#if 1
/*[IreneWang@MSTC:TO2]$ DDNS update mechanism from polling to event driven and update RG wan IP Address, 20130917*/
				if (rc == RC_OS_INVALID_IP_ADDRESS)
				{
					cmsLog_error("WAN Not Ready!");
					return rc;	/* If wan not ready, return function to wait wan up */
				}
#endif  /*1*/
			}
...
#if 1
/*[IreneWang@MSTC:TO2]$ DDNS update mechanism from polling to event driven and update RG wan IP Address, 20130917*/
			if (rc != RC_OK)	/* Only sleep when update fail */
			{
				SINT32 ret;
				struct timeval tv;
				...
				fd_set readFds;
				FD_ZERO(&readFds);
				FD_SET(handle->commFd, &readFds);
				tv.tv_sec = DYNDNS_DEFAULT_SLEEP;	/* Wait for 120 seconds*/
				tv.tv_usec = 0;
				ret = select((handle->commFd)+1, &readFds, NULL, NULL, &tv);
				if ((ret == 1) && (FD_ISSET(handle->commFd, &readFds)))
				{
					ret = RC_ERROR;		/* Stop to wait for 120s if there has event waiting outside */
				  	return ret;
				}
			}
#else
...
#if 1
				break;	/* Leaving the inner loop no matter it is ok or not */
#endif  /*1*/
			}
			else
			{
		...
		cmsLog_debug("After sleep for 120s, let's start for next retry");
	}
