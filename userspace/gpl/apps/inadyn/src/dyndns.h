...
/*[IreneWang@MSTC:TO2]$ Support DDNS, 20130227*/
#ifdef MSTC_TO2_GERMANY_CUSTOMIZATION
#define CHANGE_IP_SYSTEM_DYNAMIC  "changeip" 
#define DNSDYNAMIC_IP_SYSTEM_DYNAMIC  "dnsdynamic" 
#endif  /*MSTC_TO2_GERMANY_CUSTOMIZATION*/
...
#define GENERIC_DNSDYNAMIC_AUTH_MY_IP_REQUEST_FORMAT \
    "GET http://%s:%s@www.dnsdynamic.org%shostname=%s&myip=%s " \
	"HTTP/1.0\r\n" \
	"Authorization: Basic %s\r\n" \
	"Host: %s\r\n" \
	"User-Agent: "DYNDNS_AGENT_NAME " " DYNDNS_EMAIL_ADDR"\r\n\r\n"
...
typedef enum
{
   WAN_UP_Not_Init = -1,	/* Interface not ready . */
   WAN_UP_ALL   = 0, 		/* Interface for all wan. */
   WAN_UP_VOIP = 1, 		/* Interface for voip/bbi wan in single mode. */
   WAN_UP_BBI  = 2  		/* Interface for bbi wan in dual mode. */
} RegisterWANUpInterface;
#endif  /*MSTC_TO2_GERMANY_CUSTOMIZATION*/
