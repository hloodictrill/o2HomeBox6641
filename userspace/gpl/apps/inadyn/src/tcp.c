...
/*[IreneWang@MSTC:TO2]$ DDNS can use difference interface under single/dual mode, 20130309*/
		/* set timeouts */
/*		setsockopt(p_self->super.socket,SOL_SOCKET,SO_RCVTIMEO,
		...
		if (fPtr) {
			fscanf(fPtr, "%d", &dule);
			if (dule) {
				printf("inadyn : dule mode, use ppp0.2 interface!!\n");
				setsockopt(p_self->super.socket, SOL_SOCKET, SO_BINDTODEVICE, "ppp0.2", 6);
			}
			else {
				printf("inadyn : single mode, use ppp0.1 interface!!\n");
				setsockopt(p_self->super.socket, SOL_SOCKET, SO_BINDTODEVICE, "ppp0.1", 6);
			}
			fclose(fPtr);
		}
		else {
			printf("inadyn : file open fail!!\n");
			setsockopt(p_self->super.socket, SOL_SOCKET, SO_BINDTODEVICE, "ppp0.1", 6);
		}
