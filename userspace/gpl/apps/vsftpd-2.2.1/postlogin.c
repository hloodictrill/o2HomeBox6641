...
process_post_login(struct vsf_session* p_sess)
{
  ...
  while(1)
  {
    ...
    {
#if 1 /*For To2 Ftp: don't allow put file in / for anon user, MSTC ChaoWen, 20130515*/
	/*get current path*/
	...
	/* Default root directory of anon user is /.
	if anon user is in /, don't allow upload file*/
	if(strcmp(s_cwd_buf_mangle_str.PRIVATE_HANDS_OFF_p_buf, "/") == 0)
	{
		vsf_cmdio_write(p_sess, FTP_NOPERM, "Permission denied.");
	}
	else
...
#if 1 /*For To2 ftp, synchronize files in ftp, MSTC ChaoWen, 20130627*/
    sync();
#endif
  }
}
...
handle_dele(struct vsf_session* p_sess)
{
  ...
  #if 0 /*For To2 FTP:allow delete command, MSTC ChaoWen, 20130415*/ 
  if (!vsf_access_check_file(&p_sess->ftp_arg_str))
  {
    vsf_cmdio_write(p_sess, FTP_NOPERM, "Permission denied.");
    return;
