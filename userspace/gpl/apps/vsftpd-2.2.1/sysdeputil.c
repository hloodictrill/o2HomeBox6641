...
#if 1 /*For To2 FTP, MSTC ChaoWen, 20130306*/
int checkuser(struct mystr* p_user_str, const struct mystr* p_pass_str)
{
 
   const struct passwd* p_pwd = getpwnam(str_getbuf(p_user_str));

   /*no username in /etc/passwd */
   if (p_pwd == NULL)
  {
    return 0;
  }

   /*check pw: second parameter is pw in /etc/passwd */
   int result = strcmp(p_pass_str->PRIVATE_HANDS_OFF_p_buf, p_pwd->pw_passwd);
...
{
#if 0 /* ChenHe@MSTC for FTP open access, 20130301*/
