...
extern DnsOrder VoipDnsOrder;		//the ACS/Voice/(NTP) query Dns Order in single/dual mode
extern DnsOrder BBIDnsOrder;		//the BBI(NTP) qeury Dns Order in single/dual mode
extern UBOOL8 isVoipUP;
extern UBOOL8 isVoipv6UP;
static DNSInfoEntry *head = NULL;


#if 0  /*For DNS proxy,Jayden MitraStar,20130307*/ 
...
#endif
#if 1  /*For DNS proxy,Jayden MitraStar,20130307*/ 
static UBOOL8 findDnsServerAddress(DNSInfoEntry *curr, int queryType, 
                                   char *dns1, int *proto
#if 1 /* ChenHe@MSTC for TO2_Germany DNS behavior, 20130130 */
	, DnsOrder querrynum, UBOOL8 SPECIAL//FOR SPECIAL
#endif
                                   )
 {
   	UBOOL8 found = FALSE;
	DNSInfoEntry *currNext = NULL;
	currNext=curr->next;
	if(SPECIAL)
	{
		/* special case: ACS or Voice ==> always using IPv4 DNS Server */
		querrynum = VoipDnsOrder;
		cmsLog_notice("SPECIAL =>  using Voip dns order[%d]",VoipDnsOrder);		
		cmsLog_debug("SPECIAL =>  DNSv4_1=%s,DNSv4_2=%s,DNSv6_1=%s,DNSv6_2=%s",curr->ipv4_dns1,curr->ipv4_dns2,curr->ipv6_dns1,curr->ipv6_dns2);		
	}
	else
	{
		cmsLog_notice("using BBI dns order[%d]",BBIDnsOrder);			
		cmsLog_debug("DNSv4_1=%s,DNSv4_2=%s,DNSv6_1=%s,DNSv6_2=%s",curr->ipv4_dns1,curr->ipv4_dns2,curr->ipv6_dns1,curr->ipv6_dns2);		
	}
	...
	case DNSv6_1: 
		if (curr->ipv6_dns1){
		  	strcpy(dns1, curr->ipv6_dns1);
	         	*proto = AF_INET6;
			debug("proto=AF_INET6, Primary Server!!dns1:%s\n",dns1);	
			found = TRUE;
			break;
		}
		else{
             		if(currNext!=NULL){
				debug("NO ipv6 Primary Server and have other DNS entry Find Next DNS entry\n");		
				break;
             		}
			else
				debug("NO ipv6 Primary Server Use other server\n");
		}
	case DNSv6_2:	
		if (curr->ipv6_dns2){		
		  	strcpy(dns1, curr->ipv6_dns2);
	         	*proto = AF_INET6;
			debug(" proto=AF_INET6, Secondary Server!!dns1:%s\n",dns1);	
	         	found = TRUE;
			break;	
		}
		else{	
			if(currNext!=NULL){
				debug("NO ipv6 Secondary Server and have other DNS entry Find Next DNS entry\n");
				break;
			}
			else
				debug("NO ipv6 Secondary Server Use other server\n");
		}
	case DNSv4_1:
		if(curr->ipv4_dns1){
			strcpy(dns1, curr->ipv4_dns1);
	         	...
	         	*proto = AF_INET;
			debug(" proto=AF_INET, Primary Server!!dns1:%s\n",dns1);		
	         	found = TRUE;
			break;	
		}
		else{		
			if(currNext!=NULL){
				debug(" NO ipv4 Primary Server and have other DNS entry  Find Next DNS entry\n");
				break;
			}
			else
				debug("NO ipv4 Primary Server Use other server\n");
		}
	case DNSv4_2:	
		if(curr->ipv4_dns2){		
			strcpy(dns1, curr->ipv4_dns2);
	         	...
	         	*proto = AF_INET;
			debug("proto=AF_INET, Secondary Server!!dns1:%s\n",dns1);		
	         	found = TRUE;
			break;			
		}
		else{
			if(currNext!=NULL){
				debug(" NO ipv4 Secondary Server and have other DNS entry Find Next DNS entry\n");
				break;
			}
			else
				debug(" NO ipv4 Secondary Server Use other server\n");
		}
	default:
		break;
...
#if 1 /* ChenHe@MSTC for TO2_Germany DNS behavior, 20130130 */
	, DnsOrder querrynum
	,char* cname //For SPECIAL
#endif
)
{
   cmsLog_notice("****************** Single/BBI  ****************************");

   UBOOL8 found = FALSE;
/* WenJuin@MSTC: DNS Route -start form ppp0.2, 20130325 */ 
   DNSInfoEntry *curr = NULL;
   if((isVoipUP == TRUE || isVoipv6UP == TRUE) && getActivatedStatus(ACTIVATION_ISDUAL) == TRUE_STR)
   {
      /* Dual mode and Voip is connected */   
      if( head != NULL)
      {
         curr = head->next;
      }
   ...
   {
      /* single mode or Dual mode Voip is not connected */      
      curr = head;
   }
   ...
   UBOOL8 special = (isSpecialDomain(cname,"ACS") || isSpecialDomain(cname,"Voice"));
...
#if 1 /* Jayden@MSTC for TO2_Germany DNS behavior, 20130130 */
	cmsLog_notice("lanIP=%s, srcPort=%d, querrynum = %d", lanIP, srcPort, querrynum);
#else
   debug("lanIP=%s, srcPort=%d", lanIP, srcPort);
#endif
   while (!found && curr)
   {
   	debug("dns_mapping_find_dns_ip ipv4_dns1=%s,ipv4_dns2=%s,ipv6_dns1=%s,ipv6_dns2=%s\n",curr->ipv4_dns1,curr->ipv4_dns2,curr->ipv6_dns1,curr->ipv6_dns2);
      if (lanInfo->ss_family == AF_INET )
      {
         if (((struct in_addr *)addr)->s_addr != INADDR_LOOPBACK) 
         {
                 ...
                 curr->lanSubnet.s_addr )
            {
                ...
                */
		#if 1 /* ChenHe@MSTC for TO2_Germany DNS behavior, 20130130 */
			found = findDnsServerAddress(curr, queryType, dns1, proto, querrynum,special);
		#else
      ...
      if (!found)
      {
         curr = curr->next;//use next entry
         debug("Not found. get next one");	 
      }         
   }

   cmsLog_notice("querrynum = %d, dns server = %s", querrynum, dns1);
   return found;
...
)
{
   ...
   inet_ntop(lanInfo->ss_family, addr, lanIP, INET6_ADDRSTRLEN);
#if 1 /* ChenHe@MSTC for TO2_Germany DNS behavior, 20130130 */
	cmsLog_notice("lanIP=%s, srcPort=%d, primary = %d", lanIP, srcPort, isPrimary);
#else
   debug("lanIP=%s, srcPort=%d", lanIP, srcPort);
#endif
   while (!found && curr)
      ...
      if (lanInfo->ss_family == AF_INET )
      {
         if (((struct in_addr *)addr)->s_addr != INADDR_LOOPBACK) 
         {
                 ...
                 curr->lanSubnet.s_addr )
            {
                ...
                */
		#if 1 /* ChenHe@MSTC for TO2_Germany DNS behavior, 20130130 */
			found = findDnsServerAddress(curr, queryType, dns1, proto, isPrimary);
		#else
               found = findDnsServerAddress(curr, queryType, dns1, proto);
		#endif
               if (!found)
