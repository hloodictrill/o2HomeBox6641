...
void dns_decode_reverse_name(char *name)
{
    ...
    k = 0;
/* WenJuin@MSTC: support PTR ipv6,20130423 */
#if 1   
    char Type[BUFLEN_18] = {0};
    int PTRType = -1;	// 1:ipv4 addr,  2:ipv6 addr
    int CommonNum = 0;
    int count = 0;	
    cmsUtl_strncpy(Type, (name+cmsUtl_strlen(name)-cmsUtl_strlen(PTR_IPV4SUFFIX)), cmsUtl_strlen(PTR_IPV4SUFFIX)+1);

    if(!cmsUtl_strcmp(Type, PTR_IPV4SUFFIX))
    {
        PTRType = 1;
        CommonNum = 3;
    }
    ...
    /* WenJuin@MSTC: support PTR ipv6,20130423 */
    for( j = 0, i = 0; j<CommonNum && name[i] != 0 && i<NAME_SIZE-2; i++) if( name[i] == '.' )j++;
    if (j < CommonNum) return;

    /* start copy ip addr from last '.' */
    for( j = 0, i = 0; j<CommonNum; i++) if( name[i] == '.' )j++;
    for( ; name[i] != '.' && k<NAME_SIZE-1; i++) temp[k++] = name[i];
    if (PTRType == 1 && k < NAME_SIZE-1) temp[k++] = '.';

    /* asign source string to 0 for exclude 'in-addr.arpa' or 'ip6.arpa' */	
    if (name[i] == '.') name[i] = 0;

    for(count =1;CommonNum-count > 0; count++)
    {
        for( j = 0, i = 0; j<CommonNum-count; i++) if( name[i] == '.' )j++;
        for( ; name[i] != '.' && k<NAME_SIZE-1; i++) temp[k++] = name[i];		
        if (PTRType == 1 && k < NAME_SIZE-1)
	 {
	    temp[k++] = '.';
	 }
	 else if(PTRType == 2 && (count+1)%4==0)
	 {
	    temp[k++] = ':';
	 }
    }
