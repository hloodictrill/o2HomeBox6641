...
void dns_list_print(void)
{
  ...
  for (r = dns_request_list; r; r = r->next) {
     printf("    (%p) ID: 0x%x type=%u isDomainRt=%d Name=%s IP=%s "
            "expiry=%d switch_on_timeout=%d retx_count=%d next=%p prev=%p\n",
            r, r->message.header.id,
            (unsigned int) r->message.question[0].type, r->isDomainRt,
...
void dns_DomainRt_print(void)
{
  printf("\n===========Domain Routt list, num_entry: %d\n",num_entry);

	int i=0;
	for(i=0;i<num_entry;i++)
	{
		printf("Type: %s domainName: %s ifcName: %s gateway: %s primary: %s second: %s\n",DNSRoute[i].type,DNSRoute[i].domainName, DNSRoute[i].ifcName,
			DNSRoute[i].gateway,DNSRoute[i].primary,DNSRoute[i].secondary);
	}
