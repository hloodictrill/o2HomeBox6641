...
#ifdef MSTC_COMMON_PATCH //__MTS__, Dennis
#if 0 /*__MTS__, Amy, Support load ras code by FTP, 20111209 */
   if(userinfo.pw_uid == 0 || (accessMode == NETWORK_ACCESS_DISABLED))
#else
   if ((accessMode == NETWORK_ACCESS_DISABLED) ||
	   (accessMode == NETWORK_ACCESS_LAN_SIDE && userinfo.pw_uid == 1 ) ||
	   (accessMode == NETWORK_ACCESS_WAN_SIDE && userinfo.pw_uid != 0 ))   
#endif
#else
    if ((accessMode == NETWORK_ACCESS_DISABLED) ||
...
#endif //__MTS__, Dennis, MSTC_COMMON_PATCH
    {
       syslog(LOG_WARNING,"104051 FTP Server Login UserName or Password Error\n");
...
#if 1 /*For To2 ftp, MSTC ChaoWen, 20130116*/ 
   /*don't allow account in shell to login ftp*/ 
   if(checkuser()){
#else
...
#if 1 /*For To2 FTP, MSTC ChaoWen, 20130116*/
   if (strncmp(userinfo.pw_passwd, password, strlen(userinfo.pw_passwd)) == 0) 
      return 0;
   else
      return 1;
#else
...
#ifdef MSTC_COMMON_PATCH /* Follow 11n model to support account log. __TELEFONICA__, MSTC Stan Su, 20120220. */
	char tmpLog[512] = {0};
	sprintf(tmpLog, "User %s login from FTP successfully", userinfo.pw_name);
