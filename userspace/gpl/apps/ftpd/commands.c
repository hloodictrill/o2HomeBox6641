...
//brcm begin
#include "cms_util.h"
#ifdef MSTC_COMMON_PATCH //__MTS__, Nick Tseng
#include "cms_msg.h"
#endif
...
void control_printf(char success __attribute__((unused)), char *format, ...)
{
    ...
    va_end(val);
#ifdef SUPPORT_TELEFONICA /* In order not to be blocked, CR/LF should be contained in the same packet, __TELEFONICA__, ZyXEL YungAn, 20091015. */
    strcat(buffer, "\r\n");
...
//__MTS__, Nick Tseng, support image updates when FTPD storage feature is defined
   ...
   /* reset all of our globals before starting another download */    
    ...
    for (;;) {
...
#ifdef MSTC_COMMON_PATCH //__MTS__, Nick Tseng, Lost 1 byte after transfer completed
        ...
        if (curPtr == NULL)
        {
            ...
            {
/*__MTS__, Amy, Support load ras code by FTP, 20111209 */
#ifdef MSTC_COMMON_PATCH //__VERZION__, Autumn
...
/*__MTS__, Amy, merge from Telefonica P-870HNU-51B(102BKW0) Revision#4964*/
#ifdef SUPPORT_TELEFONICA /* If firmware or config is valid, update it automatically. __TELEFONICA__, MitraStar, Stan Su, 20110823. */
	  command_quit("QUIT");
#else
...
void command_quit(char *params __attribute__((unused)))
{
    ...
    * write the image.
    */
/*__MTS__, Amy, merge from Telefonica P-870HNU-51B(102BKW0) Revision#4992,#4998*/
#ifdef SUPPORT_TELEFONICA /* If firmware or config is valid, update it automatically. __TELEFONICA__, MitraStar, Stan Su, 20110823. */
   control_printf(SL_SUCCESS, "226 File transmission successful.");
#else
...
//__MTS__, Nick Tseng, GUI Debug Mode with FTP
	...
	if (!strcmp(parentDir, "/firmware/fw/")) {
		...
		if( imageFilepPtr == NULL ) {				  	
			...
			#ifdef MSTC_TELEFONICA_IAD
			system("killall voiceApp");
			sleep(5);
			#endif
...
#if 1 //__MTS__, Nick Tseng
void command_appe(char *filename)
{
    do_stor(filename, O_CREAT | O_WRONLY | O_APPEND); //__MTS__, Nick Tseng
}
#endif
...
#ifdef MSTC_COMMON_PATCH //__MTS__, Nick Tseng, support config backup when FTPD storage feature is defined
