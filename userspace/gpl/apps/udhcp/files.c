...
#ifdef MSTC_COMMON_PATCH //__OBM__, FuChia
static vendor_id_t * read_vendoridwildcardlist(char *line)
{
	char *token, tmp[BUFLEN_256];
	vendor_id_t *new, *header = NULL, *now;
...
static int read_opt(char *line, void *arg)
{
	...
	do {
		#ifdef MSTC_COMMON_PATCH /* Fix option does not support "," issue, __TELEFONICA__, ZyXEL YungAn, 20100927. */
                val = strtok(NULL, " \t");		
		#else
		val = strtok(NULL, ", \t");
		#endif
...
static int read_staticleasedevname(const char *const_line, void *arg)
{
	...
	/* Read devname */
	line = (char *) const_line;
	devname_string = strtok(line, " \t");
	devname = xmalloc(strlen(devname_string));
	read_str(devname_string, &devname);

	/* Read ip */
	ip_string = strtok(NULL, " \t");
	read_ip(ip_string, ip);

	addStaticLeaseDevname(arg, devname, ip);

#ifdef UDHCP_DEBUG /*For To2 fix ipaddress by device name, Mitrastar ChingLung, 20130301*/
	printStaticLeasesDevname(arg);
#endif
...
#if 1 /*For To2 fix ipaddress by device name, Mitrastar ChingLung, 20130301*/	
	struct static_lease_devname *sld_cur, *sld_next;
#endif
...
#if 1 /*For To2 fix ipaddress by device name, Mitrastar ChingLung, 20130301*/	
	sld_cur = iface->static_leases_devnames;
	while(sld_cur) {
		sld_next = sld_cur->next;
		if(sld_cur->devname)
			free(sld_cur->devname);
		if(sld_cur->ip)
			free(sld_cur->ip);
		free(sld_cur);
		sld_cur = sld_next;
...
#ifdef MSTC_COMMON_PATCH //__QWEST__, Steven
...
#if 1 /*For To2 fix ipaddress by device name, Mitrastar ChingLung, 20130301*/
		else if (strcasecmp(token, "static_lease_devname") == 0)
		{
			read_staticleasedevname(line, &cur_iface->static_leases_devnames);
		}
#endif
...
#endif
#ifdef MSTC_COMMON_PATCH //__OBM__, FuChia
            else if (strcasecmp(token, "vid_wildcard") == 0) 
			cur_iface->vendor_id_wildcards= read_vendoridwildcardlist(line);
#endif //__OBM__, FuChia
#ifdef MSTC_COMMON_PATCH // ZyXEL, Hong-Yu
...
void send_VIoption_info(UBOOL8 isDelete, struct dhcpOfferedAddr *lease)
{
	...
	DEBUG(LOG_INFO, "sending VIoption info update msg, isDelete=%d", isDelete);
#if 0 /* Amy */
	DEBUG(LOG_INFO, "assigned addr = %s", inet_ntoa(inaddr));
#endif

	hdr->type = CMS_MSG_DHCPD_VIOPTION_INFO;
	hdr->src = EID_DHCPD;
	hdr->dst = EID_SSK;
	hdr->flags_event = 1;
	...
	{
		LOG(LOG_WARNING, "could not send VI option info update");
	}
	else
	{
		DEBUG(LOG_INFO, "lVI option info info update sent!");
	}
}
#endif
...
void send_lease_info(UBOOL8 isDelete, const struct dhcpOfferedAddr *lease)
{
   ...
   body->deleteHost = isDelete;
#ifdef MSTC_COMMON_PATCH /*ZyXEL Autumn static DHCP will course error*/
   body->leaseTimeRemaining = (remaining<-1?-1:remaining);
#else
...
#ifdef MSTC_COMMON_PATCH //__Qwest__, SinJia, Fix when the inferface is condservpoolx_brx in DHCP conditional serving
...
#ifdef MTS_Telefonica_TR69	/* the VendorClassID, ClientID and UserClassID are also contained in the lease struct, __TELEFONICA__, ZyXEL Eva, 20100624. */
   snprintf(body->vendorid, sizeof(body->vendorid), lease->vendorid);
   snprintf(body->clientid, sizeof(body->clientid), lease->clientid);
   snprintf(body->userclsid, sizeof(body->userclsid), lease->classid);	
#endif
...
#ifndef MTS_DHCP //__MTS__,Lynn,DHCP
