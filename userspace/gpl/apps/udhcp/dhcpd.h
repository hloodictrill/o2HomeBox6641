...
struct static_lease_devname {
	unsigned char *devname;
	uint32_t *ip;
	struct static_lease_devname *next;
};
#endif
...
typedef struct vendor_id_wildcard_struct{
	int len;
	char *wildcardenable;
	struct vendor_id_wildcard_struct * next;
}vendor_id_wildcard_t;
...
#ifdef MSTC_TO2_GERMANY_CUSTOMIZATION /*For To2 fix ipaddress by device name, Mitrastar ChingLung, 20130301*/
	struct static_lease_devname *static_leases_devnames; /*List of ip/devname pairs to assign static leases */
#endif
	vendor_id_t *vendor_ids;	/* vendor ID list */
#ifdef MSTC_COMMON_PATCH //__OBM__, FuChia
      vendor_id_wildcard_t *vendor_id_wildcards; /*vendor ID wild card support list*/
#endif //__OBM__, FuChia
