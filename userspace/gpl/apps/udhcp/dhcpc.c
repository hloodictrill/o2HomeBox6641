...
void send6rdEventMessage(UBOOL8 assigned, const char *ip4BrAddr, const UINT32 *ip4MaskLen, const char *ip6Prefix, const UINT32 *ip6PrefixLen)
{
   char buf[sizeof(CmsMsgHeader) + sizeof(Dhcpc6rdStateChangedMsgBody)]={0};
   CmsMsgHeader *msg=(CmsMsgHeader *) buf;
   Dhcpc6rdStateChangedMsgBody *dhcpcBody = (Dhcpc6rdStateChangedMsgBody *) (msg+1);
   CmsRet ret;

   msg->type = CMS_MSG_DHCPC_6RD_STATE_CHANGED;
   msg->src = MAKE_SPECIFIC_EID(getpid(), EID_DHCPC);
   msg->dst = EID_SSK;
   msg->flags_event = 1;
   msg->dataLength = sizeof(Dhcpc6rdStateChangedMsgBody);

   dhcpcBody->addressAssigned = assigned;
   if (assigned)
   {
      dhcpcBody->ip4MaskLen = ip4MaskLen;
      sprintf(dhcpcBody->ip6Prefix, ip6Prefix);
      dhcpcBody->ip6PrefixLen = ip6PrefixLen;
      sprintf(dhcpcBody->ip4BrAddr, ip4BrAddr);
   }
   if ((ret = cmsMsg_send(msgHandle, msg)) != CMSRET_SUCCESS)
   {
      cmsLog_error("could not send out DHCPC_STATUS_CHANGED, ret=%d", ret);
...
void processCmsMsg(void)
{
   ...
   while( (ret = cmsMsg_receiveWithTimeout(msgHandle, &msg, 0)) == CMSRET_SUCCESS) {
      switch(msg->type) {
      case CMS_MSG_DHCPC_6RD_OPTION_ENABLED:
         cmsLog_debug("changed is6rdDhcpEnabled is %d",msg->wordData);
         en_212 = msg->wordData;
         break;
      default:
...
#endif
{
	...
	int pid_fd;
#ifdef MSTC_COMMON_PATCH // ZyXEL, Hong-Yu
	// set rx buffer size to 3600, it can queue about 10-20 pkt in the socket.
	int rcvbuf=3600;
#endif
#ifdef SUPPORT_IPV6_6RD //__Common__, DingRuei
	int cms_msg_fd = 0;
...
#ifdef MSTC_COMMON_PATCH // ZyXEL, Hong-Yu
#ifdef SUPPORT_IPV6_6RD //__Common__, DingRuei
		c = getopt_long(argc, argv, "c:fH:hi:np:qr:s:d:a:u:o:P:m:S:v:R:", options, &option_index);
#else
		c = getopt_long(argc, argv, "c:fH:hi:np:qr:s:d:a:u:o:P:m:S:v:6:I:D:", options, &option_index);
#endif
#else // brcm ori
...
// brcm
		...
		case 'v':
			...
			break;
#ifdef MSTC_COMMON_PATCH // ZyXEL, Hong-Yu
		/*rutWan_startDhcpc() passed option 'I' for IAID
		   KeyYang@MSTC 20120930*/
		case 'I':
		case 'a':
			strcpy(iaid, optarg);
			// printf(" >> iaid: %s <<\n", iaid);
			break;
		/*rutWan_startDhcpc() passed option 'D' for DUID
		   KeyYang@MSTC 20120930*/
		case 'D':
		case 'u':
			strcpy(duid, optarg);
...
#ifdef SUPPORT_IPV6_6RD //__Common__, DingRuei
   cmsMsg_getEventHandle(msgHandle, &cms_msg_fd);
#endif
