...
#ifdef MSTC_COMMON_PATCH // ZyXEL, Hong-Yu
	/* dhcp option 125 */
	if (strlen(oui) && strlen(product_class) && strlen(model_name) && strlen(serial_number)) {
		if (createOption125(VENDOR_IDENTIFYING_FOR_DEVICE, VSI) != -1)
...
int send_selecting(unsigned long xid, unsigned long server, unsigned long requested)
{
	...
	if(en_212)
		add_simple_option(packet.options, DHCP_6RD, 0);
#endif
