...
#ifdef SUPPORT_IPV6_6RD //__Common__, DingRuei, enabled 6RD option
extern int en_212;
#endif
// brcm
...
#ifdef SUPPORT_IPV6_6RD //__Common__, DingRuei
extern void send6rdEventMessage(UBOOL8 assigned, const char *ip4BrAddr, const UINT32 *ip4MaskLen, const char *ip6Prefix, const UINT32 *ip6PrefixLen);
#endif
...
struct in_addr select_ip4braddr(const struct in_addr *ip4addr, const UINT32 optionlen)
{
	int ip4count = (optionlen - 18) / 4;
	struct in_addr ip4BrAddr;
	struct in_addr *ptr = ip4addr;
	int i = 0;

	cmsLog_debug("optionlen(%d),ip4count(%d)", optionlen, ip4count);
...
void fill_6rdOption(struct dhcpMessage *packet)
{
	char *temp;
	UINT32 optionlen;
	UINT32 ip4MaskLen;
	...
	char ip6Prefix[40];
	char *ip4BrAddr;
	struct in_addr ip4BrAddrTemp;
	struct in6_addr ip6PrefixTemp;

#if 0  /*dhcpc get option use magic number, __TELEFONICA__, Mitrastar Smile, 20110314. */
	if ((temp = get_option(packet, options[25].code))) { //get 6RD option
#else
	if ((temp = get_option(packet, DHCP_6RD))) { //get 6RD option
#endif
		optionlen = (UINT32)temp[-1];
		ip4MaskLen = (UINT32)temp[0];
		ip6PrefixLen = (UINT32)temp[1];


		if(ip4MaskLen > MAX_IPV4_LEN || ip4MaskLen < 0)
		{
			cmsLog_error("ip4MaskLen(%d) more than to 32 or less than 0", ip4MaskLen);
			return;
		}
		if(ip6PrefixLen > MAX_IPV6_LEN || ip6PrefixLen < 0)
		{
			cmsLog_error("ip6PrefixLen(%d) more than to 128 or less than 0", ip6PrefixLen);
			return;
		}
		/* rfc5969 */
		/* (32 - IPv4MaskLen) + 6rdPrefixLen less than or equal to 128 */
		if( (MAX_IPV4_LEN - ip4MaskLen) + ip6PrefixLen > MAX_IPV6_LEN)
		{
			cmsLog_error("(32 - IPv4MaskLen) + 6rdPrefixLen more than to 128");
			return;
		}

		//TODO: Now only support 32bits prefix. It should be decided by prefix length.
		ip6PrefixTemp.s6_addr32[1] = 0;
		ip6PrefixTemp.s6_addr32[2] = 0;
		ip6PrefixTemp.s6_addr32[3] = 0;
		memcpy(&ip6PrefixTemp.s6_addr32[0], &temp[2], 4);

		inet_ntop(AF_INET6, &ip6PrefixTemp, &ip6Prefix, sizeof(ip6Prefix));
		ip4BrAddrTemp = select_ip4braddr(&temp[18], optionlen);
		ip4BrAddr = inet_ntoa(ip4BrAddrTemp);

		send6rdEventMessage(TRUE, ip4BrAddr, ip4MaskLen, ip6Prefix, ip6PrefixLen);

		cmsLog_debug("ip6Prefix %s, ip4BrAddr %s", ip6Prefix, ip4BrAddr);
	}
...
void sipServer(char *temp)
{
	...
	strcpy(dhcpc_opt.sipServer,temp2);
...
#ifdef BUILD_VOICE_OPTION_SIPSERVER
	/* Jennifer, test, RFC3361*/
	temp = (char *)get_option(packet, DHCP_SIP_SERVERS);
	if(temp!=NULL)
	{
		/*temp[-1] is length, temp[0] is enc*/				
		if(temp[0]==0)
		{
			sipServer(temp);
		}
		...
		{
			sprintf(dhcpc_opt.sipServer, "%u.%u.%u.%u",temp[1],temp[2],temp[3],temp[4]);
		}
	}
	else
	{
		strcpy(dhcpc_opt.sipServer, "");
	}
#endif
