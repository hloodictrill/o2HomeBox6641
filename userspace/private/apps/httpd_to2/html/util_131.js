...
function isValidIpAddress(address) {
   ...
   for (i = 0; i < 4; i++) {
      if (isNaN(addrParts[i]) || addrParts[i] =="")
         return false;
	  if(!reg_number.test(addrParts[i]))
	  {
		return false;
	  }
...
/*do not encode german, MSTC ChaoWen, 20131029*/
function encodeUrl_german(val)
{
   var len     = val.length;
   var i       = 0;
   var newStr  = "";
   ...
   for ( i = 0; i < len; i++ ) {
      if ( val.substring(i,i+1).charCodeAt(0) < 255 ) {
         ...
         if (isUnsafe(val.substring(i,i+1)) == false)
			...
			else if(str.test(val.substring(i,i+1))== true)
			{
				newStr = newStr + val.substring(i,i+1);
			}	
         else
...
/*For wifi check ssid, MSTC ChaoWen, 2013917*/
...
function ssidAllowStr()
{
	var str;
	
	str ="Der Name des Funknetzes darf nur aus den Zeichen [0-9], [a-z], [A-Z], [!@#~%-+=,$%^&*()_{}[]\\<>?/.`:\';\"|] bestehen.";
