...
all_postcheck1: profile_saved_check sanity_check \
     ...
     userspace gdbserver vodsl callmngr zyims isdnCmd dectd zydect hosttools buildimage
...
# build specific application at BRCM
ifeq ($(strip $(BUILD_MSTC_COMMON_PATCH)),y)
option=
...
httpd: sanity_check create_install data-model
	$(MAKE) -C $(BUILD_DIR)/userspace/private/apps/httpd_to2
else
httpd: sanity_check create_install data-model
...
create_install:
		...
		mkdir -p $(INSTALL_DIR)/AnnounceFile
...
callmngr: sanity_check
	$(MAKE) -C $(BUILD_DIR)/userspace/stollmann
else
callmngr: sanity_check
	@echo "Skipping CallMngr+ application (not configured)"
endif

# ZyIMS
ifneq ($(strip $(BUILD_MSTC_VOIP_ZYIMS)),)
zyims: sanity_check
	$(MAKE) -C $(BUILD_DIR)/userspace/ZyXEL/ZyIMS
else
zyims: sanity_check
...
phonetest: sanity_check
	$(MAKE) -C $(BUILD_DIR)/userspace/private/apps/phonetest
else
phonetest: sanity_check
	@echo "skipping phonetest (not configured)"
endif

#isdnCmd
isdnCmd: sanity_check
	$(MAKE) -C $(BUILD_DIR)/userspace/ZyXEL/isdnCmd
...
# ZyIMS DECT
zydect: sanity_check
	$(MAKE) -C $(BUILD_DIR)/userspace/ZyXEL/ZyIMS/dspg_itf/test
else
zydect: sanity_check
	@echo "skipping zydect (not configured)"
endif
...
# star __MTS__, RaynorChung: Support 963268, patch form SVN#3597 on http://svn.zyxel.com.tw/svn/CPE_SW1/BCM96368/trunk/P-870HA/branches/cht/fttb8/4.11 
	...
	cd $(PROFILE_DIR); \
  if [ "$(BRCM_DEFAULTCFG)" != """" ]; then \
  $(HOSTTOOLS_DIR)/CompressConfig $(DEFAULTCFG_DIR)/$(BRCM_DEFAULTCFG) $(PROFILE_DIR)/$(BRCM_DEFAULTCFG); \
  fi ; \
  if [ "$(BUILD_HPNA)" = "y" ]; then \
      if [ "$(ZYXEL_OBM_IMAGE_DEFAULT)" = "y" ]; then \
		$(HOSTTOOLS_DIR)/bcmZyNandImageBuilder --output $(FS_KERNEL_IMAGE_NAME)_NAND128 --chip $(BRCM_CHIP) --board $(BRCM_BOARD_ID) --internalversion=$(ZYXEL_INTERNAL_VERSION) --externalversion=$(ZYXEL_EXTERNAL_VERSION) --modelid=$(ZYXEL_MODEL_ID) --cmodelid=$(ZYXEL_CUSTOM_MODEL_ID) --imgdefaultfile=$(BRCM_DEFAULTCFG) --hpnaswversion=$(ZYXEL_HPNA_SWVERSION) --hpnafile $(TARGETS_DIR)/HPNA/flash.bin --cfefile $(CFE_NAND128_FILE) --rootfsfile rootfs128kb.img --kernelfile vmlinux.lz; \
		$(HOSTTOOLS_DIR)/bcmZyNandImageBuilder --output $(CFE_FS_KERNEL_IMAGE_NAME)_NAND128 --chip $(BRCM_CHIP) --board $(BRCM_BOARD_ID) --internalversion=$(ZYXEL_INTERNAL_VERSION) --externalversion=$(ZYXEL_EXTERNAL_VERSION) --modelid=$(ZYXEL_MODEL_ID) --cmodelid=$(ZYXEL_CUSTOM_MODEL_ID) --imgdefaultfile=$(BRCM_DEFAULTCFG) --hpnaswversion=$(ZYXEL_HPNA_SWVERSION) --hpnafile $(TARGETS_DIR)/HPNA/flash.bin --cfefile $(CFE_NAND128_FILE) --rootfsfile rootfs128kb.img --kernelfile vmlinux.lz --include-cfe; \
		$(HOSTTOOLS_DIR)/createZyNandimg --boardid=$(BRCM_BOARD_ID) --numbermac=$(BRCM_NUM_MAC_ADDRESSES) --macaddr=$(BRCM_BASE_MAC_ADDRESS) --countrycode=$(ZYXEL_COUNTRY_CODE) --tp=$(BRCM_MAIN_TP_NUM) --psisize=$(BRCM_PSI_SIZE) --gponsn=$(BRCM_GPON_SERIAL_NUMBER) --gponpw=$(BRCM_GPON_PASSWORD) --backuppsi="$(BUILD_BACKUP_PSI)" --inputfile=$(CFE_FS_KERNEL_IMAGE_NAME)_NAND128 --outputfile=$(FLASH_IMAGE_NAME)_NAND128; \
		$(HOSTTOOLS_DIR)/bcmZyNandImageBuilder --output $(CFE_FS_KERNEL_IMAGE_NAME)_NAND128_TE --chip $(BRCM_CHIP) --board $(BRCM_BOARD_ID) --internalversion=$(ZYXEL_INTERNAL_VERSION) --externalversion=$(ZYXEL_EXTERNAL_VERSION) --modelid=$(ZYXEL_MODEL_ID) --cmodelid=$(ZYXEL_CUSTOM_MODEL_ID) --imgdefaultfile=$(BRCM_DEFAULTCFG) --hpnaswversion=$(ZYXEL_HPNA_SWVERSION) --hpnafile $(TARGETS_DIR)/HPNA/flash.bin --cfefile $(CFE_NAND128_FILE_TE) --rootfsfile rootfs128kb.img --kernelfile vmlinux.lz --include-cfe; \
		$(HOSTTOOLS_DIR)/createZyNandimg --boardid=$(BRCM_BOARD_ID) --numbermac=$(BRCM_NUM_MAC_ADDRESSES) --macaddr=$(BRCM_BASE_MAC_ADDRESS) --countrycode=$(ZYXEL_COUNTRY_CODE) --tp=$(BRCM_MAIN_TP_NUM) --psisize=$(BRCM_PSI_SIZE) --gponsn=$(BRCM_GPON_SERIAL_NUMBER) --gponpw=$(BRCM_GPON_PASSWORD) --backuppsi="$(BUILD_BACKUP_PSI)" --inputfile=$(CFE_FS_KERNEL_IMAGE_NAME)_NAND128_TE --outputfile=$(FLASH_IMAGE_NAME)_NAND128_TE; \
      else \
		$(HOSTTOOLS_DIR)/bcmZyNandImageBuilder --output $(FS_KERNEL_IMAGE_NAME)_NAND128 --chip $(BRCM_CHIP) --board $(BRCM_BOARD_ID) --internalversion=$(ZYXEL_INTERNAL_VERSION) --externalversion=$(ZYXEL_EXTERNAL_VERSION) --modelid=$(ZYXEL_MODEL_ID) --cmodelid=$(ZYXEL_CUSTOM_MODEL_ID) --hpnaswversion=$(ZYXEL_HPNA_SWVERSION) --hpnafile $(TARGETS_DIR)/HPNA/flash.bin --cfefile $(CFE_NAND128_FILE) --rootfsfile rootfs128kb.img --kernelfile vmlinux.lz; \
		$(HOSTTOOLS_DIR)/bcmZyNandImageBuilder --output $(CFE_FS_KERNEL_IMAGE_NAME)_NAND128 --chip $(BRCM_CHIP) --board $(BRCM_BOARD_ID) --internalversion=$(ZYXEL_INTERNAL_VERSION) --externalversion=$(ZYXEL_EXTERNAL_VERSION) --modelid=$(ZYXEL_MODEL_ID) --cmodelid=$(ZYXEL_CUSTOM_MODEL_ID) --hpnaswversion=$(ZYXEL_HPNA_SWVERSION) --hpnafile $(TARGETS_DIR)/HPNA/flash.bin --cfefile $(CFE_NAND128_FILE) --rootfsfile rootfs128kb.img --kernelfile vmlinux.lz --include-cfe; \
		$(HOSTTOOLS_DIR)/createZyNandimg --boardid=$(BRCM_BOARD_ID) --numbermac=$(BRCM_NUM_MAC_ADDRESSES) --macaddr=$(BRCM_BASE_MAC_ADDRESS) --countrycode=$(ZYXEL_COUNTRY_CODE) --tp=$(BRCM_MAIN_TP_NUM) --psisize=$(BRCM_PSI_SIZE) --gponsn=$(BRCM_GPON_SERIAL_NUMBER) --gponpw=$(BRCM_GPON_PASSWORD) --backuppsi="$(BUILD_BACKUP_PSI)" --inputfile=$(CFE_FS_KERNEL_IMAGE_NAME)_NAND128 --outputfile=$(FLASH_IMAGE_NAME)_NAND128; \
		$(HOSTTOOLS_DIR)/bcmZyNandImageBuilder --output $(CFE_FS_KERNEL_IMAGE_NAME)_NAND128_TE --chip $(BRCM_CHIP) --board $(BRCM_BOARD_ID) --internalversion=$(ZYXEL_INTERNAL_VERSION) --externalversion=$(ZYXEL_EXTERNAL_VERSION) --modelid=$(ZYXEL_MODEL_ID) --cmodelid=$(ZYXEL_CUSTOM_MODEL_ID) --hpnaswversion=$(ZYXEL_HPNA_SWVERSION) --hpnafile $(TARGETS_DIR)/HPNA/flash.bin --cfefile $(CFE_NAND128_FILE_TE) --rootfsfile rootfs128kb.img --kernelfile vmlinux.lz --include-cfe; \
		$(HOSTTOOLS_DIR)/createZyNandimg --boardid=$(BRCM_BOARD_ID) --numbermac=$(BRCM_NUM_MAC_ADDRESSES) --macaddr=$(BRCM_BASE_MAC_ADDRESS) --countrycode=$(ZYXEL_COUNTRY_CODE) --tp=$(BRCM_MAIN_TP_NUM) --psisize=$(BRCM_PSI_SIZE) --gponsn=$(BRCM_GPON_SERIAL_NUMBER) --gponpw=$(BRCM_GPON_PASSWORD) --backuppsi="$(BUILD_BACKUP_PSI)" --inputfile=$(CFE_FS_KERNEL_IMAGE_NAME)_NAND128_TE --outputfile=$(FLASH_IMAGE_NAME)_NAND128_TE; \
      fi;\
  else \
      if [ "$(ZYXEL_OBM_IMAGE_DEFAULT)" = "y" ]; then \
		$(HOSTTOOLS_DIR)/bcmZyNandImageBuilder --output $(FS_KERNEL_IMAGE_NAME)_NAND128 --chip $(BRCM_CHIP) --board $(BRCM_BOARD_ID) --internalversion=$(ZYXEL_INTERNAL_VERSION) --externalversion=$(ZYXEL_EXTERNAL_VERSION) --modelid=$(ZYXEL_MODEL_ID) --cmodelid=$(ZYXEL_CUSTOM_MODEL_ID) --imgdefaultfile=$(BRCM_DEFAULTCFG) --cfefile $(CFE_NAND128_FILE) --rootfsfile rootfs128kb.img --kernelfile vmlinux.lz; \
		$(HOSTTOOLS_DIR)/bcmZyNandImageBuilder --output $(CFE_FS_KERNEL_IMAGE_NAME)_NAND128 --chip $(BRCM_CHIP) --board $(BRCM_BOARD_ID) --internalversion=$(ZYXEL_INTERNAL_VERSION) --externalversion=$(ZYXEL_EXTERNAL_VERSION) --modelid=$(ZYXEL_MODEL_ID) --cmodelid=$(ZYXEL_CUSTOM_MODEL_ID) --imgdefaultfile=$(BRCM_DEFAULTCFG) --cfefile $(CFE_NAND128_FILE) --rootfsfile rootfs128kb.img --kernelfile vmlinux.lz --include-cfe; \
		$(HOSTTOOLS_DIR)/createZyNandimg --boardid=$(BRCM_BOARD_ID) --numbermac=$(BRCM_NUM_MAC_ADDRESSES) --macaddr=$(BRCM_BASE_MAC_ADDRESS) --countrycode=$(ZYXEL_COUNTRY_CODE) --tp=$(BRCM_MAIN_TP_NUM) --psisize=$(BRCM_PSI_SIZE) --gponsn=$(BRCM_GPON_SERIAL_NUMBER) --gponpw=$(BRCM_GPON_PASSWORD) --backuppsi="$(BUILD_BACKUP_PSI)" --inputfile=$(CFE_FS_KERNEL_IMAGE_NAME)_NAND128 --outputfile=$(FLASH_IMAGE_NAME)_NAND128; \
		$(HOSTTOOLS_DIR)/bcmZyNandImageBuilder --output $(CFE_FS_KERNEL_IMAGE_NAME)_NAND128_TE --chip $(BRCM_CHIP) --board $(BRCM_BOARD_ID) --internalversion=$(ZYXEL_INTERNAL_VERSION) --externalversion=$(ZYXEL_EXTERNAL_VERSION) --modelid=$(ZYXEL_MODEL_ID) --cmodelid=$(ZYXEL_CUSTOM_MODEL_ID) --imgdefaultfile=$(BRCM_DEFAULTCFG) --cfefile $(CFE_NAND128_FILE_TE) --rootfsfile rootfs128kb.img --kernelfile vmlinux.lz --include-cfe; \
		$(HOSTTOOLS_DIR)/createZyNandimg --boardid=$(BRCM_BOARD_ID) --numbermac=$(BRCM_NUM_MAC_ADDRESSES) --macaddr=$(BRCM_BASE_MAC_ADDRESS) --countrycode=$(ZYXEL_COUNTRY_CODE) --tp=$(BRCM_MAIN_TP_NUM) --psisize=$(BRCM_PSI_SIZE) --gponsn=$(BRCM_GPON_SERIAL_NUMBER) --gponpw=$(BRCM_GPON_PASSWORD) --backuppsi="$(BUILD_BACKUP_PSI)" --inputfile=$(CFE_FS_KERNEL_IMAGE_NAME)_NAND128_TE --outputfile=$(FLASH_IMAGE_NAME)_NAND128_TE; \
      else \
		$(HOSTTOOLS_DIR)/bcmZyNandImageBuilder --output $(FS_KERNEL_IMAGE_NAME)_NAND128 --chip $(BRCM_CHIP) --board $(BRCM_BOARD_ID) --internalversion=$(ZYXEL_INTERNAL_VERSION) --externalversion=$(ZYXEL_EXTERNAL_VERSION) --modelid=$(ZYXEL_MODEL_ID) --cmodelid=$(ZYXEL_CUSTOM_MODEL_ID) --cfefile $(CFE_NAND128_FILE) --rootfsfile rootfs128kb.img --kernelfile vmlinux.lz; \
		$(HOSTTOOLS_DIR)/bcmZyNandImageBuilder --output $(CFE_FS_KERNEL_IMAGE_NAME)_NAND128 --chip $(BRCM_CHIP) --board $(BRCM_BOARD_ID) --internalversion=$(ZYXEL_INTERNAL_VERSION) --externalversion=$(ZYXEL_EXTERNAL_VERSION) --modelid=$(ZYXEL_MODEL_ID) --cmodelid=$(ZYXEL_CUSTOM_MODEL_ID) --cfefile $(CFE_NAND128_FILE) --rootfsfile rootfs128kb.img --kernelfile vmlinux.lz --include-cfe; \
		$(HOSTTOOLS_DIR)/createZyNandimg --boardid=$(BRCM_BOARD_ID) --numbermac=$(BRCM_NUM_MAC_ADDRESSES) --macaddr=$(BRCM_BASE_MAC_ADDRESS) --countrycode=$(ZYXEL_COUNTRY_CODE) --tp=$(BRCM_MAIN_TP_NUM) --psisize=$(BRCM_PSI_SIZE) --gponsn=$(BRCM_GPON_SERIAL_NUMBER) --gponpw=$(BRCM_GPON_PASSWORD) --backuppsi="$(BUILD_BACKUP_PSI)" --inputfile=$(CFE_FS_KERNEL_IMAGE_NAME)_NAND128 --outputfile=$(FLASH_IMAGE_NAME)_NAND128; \
		$(HOSTTOOLS_DIR)/bcmZyNandImageBuilder --output $(CFE_FS_KERNEL_IMAGE_NAME)_NAND128_TE --chip $(BRCM_CHIP) --board $(BRCM_BOARD_ID) --internalversion=$(ZYXEL_INTERNAL_VERSION) --externalversion=$(ZYXEL_EXTERNAL_VERSION) --modelid=$(ZYXEL_MODEL_ID) --cmodelid=$(ZYXEL_CUSTOM_MODEL_ID) --cfefile $(CFE_NAND128_FILE_TE) --rootfsfile rootfs128kb.img --kernelfile vmlinux.lz --include-cfe; \
		$(HOSTTOOLS_DIR)/createZyNandimg --boardid=$(BRCM_BOARD_ID) --numbermac=$(BRCM_NUM_MAC_ADDRESSES) --macaddr=$(BRCM_BASE_MAC_ADDRESS) --countrycode=$(ZYXEL_COUNTRY_CODE) --tp=$(BRCM_MAIN_TP_NUM) --psisize=$(BRCM_PSI_SIZE) --gponsn=$(BRCM_GPON_SERIAL_NUMBER) --gponpw=$(BRCM_GPON_PASSWORD) --backuppsi="$(BUILD_BACKUP_PSI)" --inputfile=$(CFE_FS_KERNEL_IMAGE_NAME)_NAND128_TE --outputfile=$(FLASH_IMAGE_NAME)_NAND128_TE; \
      fi;\
  fi;\
	if [ "$(BRCM_DEFAULTCFG)" != """" ]; then \
    rm $(PROFILE_DIR)/$(BRCM_DEFAULTCFG); \
    fi;\
	if [ "$(ZYXEL_NAND_BLOCK_16)" = "y" ]; then \
		$(HOSTTOOLS_DIR)/NANDimg_gen -v $(ZYXEL_EXTERNAL_VERSION) -q 1 -t small -M 1; \
		$(HOSTTOOLS_DIR)/NANDimg_gen -f $(FLASH_IMAGE_NAME)_NAND128_TE -v $(ZYXEL_EXTERNAL_VERSION) -i $(shell date '+%y%m%d_%H%M') -S 0 -s jffs2 -c on -q 1 -p $(ZYXEL_NAND_FLASH_SIZE) -t small -o brcom -e 512 -G 50; \
	else \
		$(HOSTTOOLS_DIR)/NANDimg_gen -v $(ZYXEL_EXTERNAL_VERSION) -q 1 -t large -M 1; \
		$(HOSTTOOLS_DIR)/NANDimg_gen -f $(FLASH_IMAGE_NAME)_NAND128_TE -v $(ZYXEL_EXTERNAL_VERSION) -i $(shell date '+%y%m%d_%H%M') -S 0 -s jffs2 -c on -q 1 -p $(ZYXEL_NAND_FLASH_SIZE) -t large -o brcom -e 512 -G 50; \
	fi ; \
...
ifeq ($(strip $(BRCM_KERNEL_ROOTFS)),nfs)
	...
	$(HOSTTOOLS_DIR)/bcmImageBuilder --output $(FS_KERNEL_IMAGE_NAME) --chip $(BRCM_CHIP) --board $(BRCM_BOARD_ID) --blocksize $(BRCM_FLASHBLK_SIZE) --internalversion=$(ZYXEL_INTERNAL_VERSION) --externalversion=$(ZYXEL_EXTERNAL_VERSION) --modelid=$(ZYXEL_MODEL_ID) --cmodelid=$(ZYXEL_CUSTOM_MODEL_ID) --cfefile $(CFE_FILE) --rootfsfile rootfs.img --kernelfile vmlinux.lz; \
	$(HOSTTOOLS_DIR)/bcmImageBuilder --output $(CFE_FS_KERNEL_IMAGE_NAME) --chip $(BRCM_CHIP) --board $(BRCM_BOARD_ID) --blocksize $(BRCM_FLASHBLK_SIZE) --internalversion=$(ZYXEL_INTERNAL_VERSION) --externalversion=$(ZYXEL_EXTERNAL_VERSION) --modelid=$(ZYXEL_MODEL_ID) --cmodelid=$(ZYXEL_CUSTOM_MODEL_ID) --cfefile $(CFE_FILE) --rootfsfile rootfs.img --kernelfile vmlinux.lz --include-cfe; \
	$(HOSTTOOLS_DIR)/createimg --boardid=$(BRCM_BOARD_ID) --numbermac=$(BRCM_NUM_MAC_ADDRESSES) --macaddr=$(BRCM_BASE_MAC_ADDRESS) --countrycode=$(ZYXEL_COUNTRY_CODE) --tp=$(BRCM_MAIN_TP_NUM) --psisize=$(BRCM_PSI_SIZE) --gponsn=$(BRCM_GPON_SERIAL_NUMBER) --gponpw=$(BRCM_GPON_PASSWORD) --backuppsi="$(BUILD_BACKUP_PSI)" --inputfile=$(CFE_FS_KERNEL_IMAGE_NAME) --outputfile=$(FLASH_IMAGE_NAME); \
	$(HOSTTOOLS_DIR)/addvtoken $(FLASH_IMAGE_NAME) $(FLASH_IMAGE_NAME).w
	@mkdir -p $(IMAGES_DIR)
...
rebindimg merge:
	...
	if [ "$(BRCM_DEFAULTCFG)" != """" ]; then \
		...
		if [ "$(ZYXEL_OBM_IMAGE_DEFAULT)" = "y" ]; then \
			$(HOSTTOOLS_DIR)/bcmZyNandImageBuilder --output $(FS_KERNEL_IMAGE_NAME)_NAND128 --chip $(BRCM_CHIP) --board $(BRCM_BOARD_ID) --internalversion=$(ZYXEL_INTERNAL_VERSION) --externalversion=$(ZYXEL_EXTERNAL_VERSION) --modelid=$(ZYXEL_MODEL_ID) --cmodelid=$(ZYXEL_CUSTOM_MODEL_ID) --cfefile $(CFE_NAND128_FILE) --rootfsfile rootfs128kb.img --kernelfile vmlinux.lz --imgdefaultfile=$(BRCM_DEFAULTCFG); \
			$(HOSTTOOLS_DIR)/bcmZyNandImageBuilder --output $(CFE_FS_KERNEL_IMAGE_NAME)_NAND128 --chip $(BRCM_CHIP) --board $(BRCM_BOARD_ID) --internalversion=$(ZYXEL_INTERNAL_VERSION) --externalversion=$(ZYXEL_EXTERNAL_VERSION) --modelid=$(ZYXEL_MODEL_ID) --cmodelid=$(ZYXEL_CUSTOM_MODEL_ID) --cfefile $(CFE_NAND128_FILE) --rootfsfile rootfs128kb.img --kernelfile vmlinux.lz --include-cfe --imgdefaultfile=$(BRCM_DEFAULTCFG); \
			$(HOSTTOOLS_DIR)/createZyNandimg --boardid=$(BRCM_BOARD_ID) --numbermac=$(BRCM_NUM_MAC_ADDRESSES) --macaddr=$(BRCM_BASE_MAC_ADDRESS) --countrycode=$(ZYXEL_COUNTRY_CODE) --tp=$(BRCM_MAIN_TP_NUM) --psisize=$(BRCM_PSI_SIZE) --gponsn=$(BRCM_GPON_SERIAL_NUMBER) --gponpw=$(BRCM_GPON_PASSWORD) --backuppsi="$(BUILD_BACKUP_PSI)" --inputfile=$(CFE_FS_KERNEL_IMAGE_NAME)_NAND128 --outputfile=$(FLASH_IMAGE_NAME)_NAND128; \
	    $(HOSTTOOLS_DIR)/bcmZyNandImageBuilder --output $(CFE_FS_KERNEL_IMAGE_NAME)_NAND128_TE --chip $(BRCM_CHIP) --board $(BRCM_BOARD_ID) --internalversion=$(ZYXEL_INTERNAL_VERSION) --externalversion=$(ZYXEL_EXTERNAL_VERSION) --modelid=$(ZYXEL_MODEL_ID) --cmodelid=$(ZYXEL_CUSTOM_MODEL_ID) --imgdefaultfile=$(BRCM_DEFAULTCFG) --cfefile $(CFE_NAND128_FILE_TE) --rootfsfile rootfs128kb.img --kernelfile vmlinux.lz --include-cfe; --imgdefaultfile=$(BRCM_DEFAULTCFG); \
	    $(HOSTTOOLS_DIR)/createZyNandimg --boardid=$(BRCM_BOARD_ID) --numbermac=$(BRCM_NUM_MAC_ADDRESSES) --macaddr=$(BRCM_BASE_MAC_ADDRESS) --countrycode=$(ZYXEL_COUNTRY_CODE) --tp=$(BRCM_MAIN_TP_NUM) --psisize=$(BRCM_PSI_SIZE) --gponsn=$(BRCM_GPON_SERIAL_NUMBER) --gponpw=$(BRCM_GPON_PASSWORD) --backuppsi="$(BUILD_BACKUP_PSI)" --inputfile=$(CFE_FS_KERNEL_IMAGE_NAME)_NAND128_TE --outputfile=$(FLASH_IMAGE_NAME)_NAND128_TE; \
		else \
			$(HOSTTOOLS_DIR)/bcmZyNandImageBuilder --output $(FS_KERNEL_IMAGE_NAME)_NAND128 --chip $(BRCM_CHIP) --board $(BRCM_BOARD_ID) --internalversion=$(ZYXEL_INTERNAL_VERSION) --externalversion=$(ZYXEL_EXTERNAL_VERSION) --modelid=$(ZYXEL_MODEL_ID) --cmodelid=$(ZYXEL_CUSTOM_MODEL_ID) --cfefile $(CFE_NAND128_FILE) --rootfsfile rootfs128kb.img --kernelfile vmlinux.lz; \
			$(HOSTTOOLS_DIR)/bcmZyNandImageBuilder --output $(CFE_FS_KERNEL_IMAGE_NAME)_NAND128 --chip $(BRCM_CHIP) --board $(BRCM_BOARD_ID) --internalversion=$(ZYXEL_INTERNAL_VERSION) --externalversion=$(ZYXEL_EXTERNAL_VERSION) --modelid=$(ZYXEL_MODEL_ID) --cmodelid=$(ZYXEL_CUSTOM_MODEL_ID) --cfefile $(CFE_NAND128_FILE) --rootfsfile rootfs128kb.img --kernelfile vmlinux.lz --include-cfe; \
...
clean: target_clean bcmdrivers_clean data-model_clean userspace_clean kernel_clean \
       hosttools_clean voice_clean xchange_clean wlan_clean zydect_clean
...
# callmngr
ifneq ($(strip $(BUILD_STOLLMANN_CALLMNGR)),)
callmngr_clean: sanity_check
	$(MAKE) -C $(BUILD_DIR)/userspace/stollmann clean
else
callmngr_clean:
	@echo "Skipping clean CallMngr+ application (not configured)"
endif

# ZyIMS
ifneq ($(strip $(BUILD_MSTC_VOIP_ZYIMS)),)
zyimsclean: sanity_check zydect_clean
	$(MAKE) -C $(BUILD_DIR)/userspace/ZyXEL/ZyIMS clean
else
zyimsclean:
	@echo "skipping zyims (not configured)"
endif
...
ifneq ($(strip $(BUILD_PHONETEST)),)
phonetest_clean: sanity_check
	$(MAKE) -C $(BUILD_DIR)/userspace/private/apps/phonetest clean
else
phonetest_clean: sanity_check
	@echo "Skipping phonetest application (not configured)"
endif

isdnCmd_clean: sanity_check
	$(MAKE) -C $(BUILD_DIR)/userspace/ZyXEL/isdnCmd clean
...
ifneq ($(strip $(BUILD_MSTC_VOIP_ZYIMS)),)
zydect_clean:
	$(MAKE) -C $(BUILD_DIR)/userspace/ZyXEL/ZyIMS/dspg_itf/test clean
	rm -fr $(INSTALL_DIR)/bin/cmbs_tcx
else
zydect_clean:
	@echo "Skipping zydect clean (not configured)"
endif

voice_clean: sanity_check callmngr_clean zyimsclean vodsl_clean data-model_clean dectd_clean phonetest_clean isdnCmd_clean
...
# build specific application at BRCM
ifeq ($(strip $(BUILD_MSTC_COMMON_PATCH)),y)
%: %/Makefile sanity_check create_install data-model $(BCM_SWVERSION_FILE)
	@case "${option}" in \
                all) \
                        ${MAKE} -C $@ -f Makefile; \
                        ;; \
                clean) \
                        ${MAKE} -C $@ -f Makefile ${option}; \
                        ;; \
		install) \
			${MAKE} -C $@ -f Makefile ${option}; \
			;; \
	...
	@echo "$(LINE_SEP)"
	@echo "==> Build applications $@ by option <${option}>."
endif
...
BUILD_MTS_WPS_BTN_SCE          \
BUILD_MTS_WPS_WLAN_ONE_LED     \
BUILD_MTS_WLAN_BTN_PER         \
BUILD_LLTD                 \
BUILD_WSC                   \
BUILD_BCMCRYPTO \
BUILD_BCMSHARED \
...
BUILD_IQCTL                \
BUILD_BPMCTL               \
BUILD_TELEFONICA_MHS_GUI   \
BUILD_TELEFONICA           \
MSTC_TELEFONICA_IAD	\
BUILD_MTS_WPS2_SUPPORTED\
BUILD_MSTC_SFTP_SCP        \
BUILD_MSTC_SFTP_SCP_FOR_MSTCTEST_KEY \
IMAGE_VERSION
