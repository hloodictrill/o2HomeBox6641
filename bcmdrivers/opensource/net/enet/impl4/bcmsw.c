...
{
#if !defined(MSTC_COMMON_PATCH) && !defined(CONFIG_BCM_EXT_SWITCH)
    int i;
#endif
    uint16 map;
...
/*[BUGFIXED][CASEID#581301]Can't forward packets from external switch to internal giga phy.
	ROOTCASUE: Port6 is connected to 53125 on FTTH board. The disable learning is enable on port6 (reg 0x3c=0x140). 
				It looks like if you connect PC on 53125 when boot, both 53125 and internal switch learn the MAC. 
				The MAC should not  be learn internal switch. 
	SOLUTION: Disables learning on port6(reg 0x3c bit6=1) and do fast aging to remove all MACs.
	KeyYang@MSTC 20121112*/
#if defined(MSTC_COMMON_PATCH) && defined(CONFIG_BCM_EXT_SWITCH)
	fast_age_all(1) ;
#else
...
/* Support Quantenna 5GHz Wifi module on RGMII __TELEFONICA__, MTSC Curtis, 20111212. */
#ifdef BUILD_5G 
#ifdef BUILD_MSTC_FW_WITH_DIFFERENT_HW
	if(i == WIFI_RGMII_PORT && BpGetHWType() == HW_5G_IAD) {
#else
