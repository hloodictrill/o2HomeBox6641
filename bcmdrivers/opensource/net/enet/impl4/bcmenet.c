...
static unsigned long VoIPTxCounter = 0;
...
#if 1 /*For To2's vport<--->phyport mapping,Mitrastar Jayden,20130226*/
      /* vport    -> j: eth0 | eth1 | eth2 | eth3 |	   */
      /* phyport -> i: port4| port3| port2| port1| 	   */
      if(i == 1)
      {
...
/*[MSTC Customization]Compatible Hardware Setting for Telefonica FTTH C0 Part.
i). Telefonica FTTH C0(63169 C0):  Support HPNA or 5G only.
ii). Telefonica FTTH C0(63169 D0):  Support HPNA and 5G simultaneously.
	Based on i). eth4 was configured for WAN, and eth5 was configure for WiFi 5G by BRCM default init.
	Based on ii). eth4 was configured for HPNA, eth5 was configure for WAN, and eth6 was configured for WiFi 5G by BRCM default init.
iii). For compatible	hardware issue; devname rename as 
	HPNA:	eth4 -> eth6
	WAN:	eth5 -> eth4 
	WiFi5G:	eth6 -> eth5 
Note:
	...
	i5:eth5(WiFi5G)
	i6:eth6(HPNA)
KeyYang@MSTC 20120919
*/
#if defined(BUILD_5G) && defined(BUILD_HPNA) && defined(MSTC_EXTERNAL_HPNA)
	if(i>0){
		if(MSTC_TELEFONICA_FTTH_WAN_PORT == (i-1)){
			sprintf(dev->name, "eth4"); 
		}else if(MSTC_TELEFONICA_FTTH_WIFI5G_PORT == (i-1)){
			sprintf(dev->name, "eth5"); 
		}else if(MSTC_TELEFONICA_FTTH_HPNA_PORT == (i-1)){
			sprintf(dev->name, "eth6"); 
		}else{
			/*Nothing to do*/
		}
	}
...
/*__MTSC__, Delon Yu, Support HPNA*/
	...
	if(sw_port == 11){		
		if(linkstatus){ /*eth4 UP*/
	#if 1 /*For To2 Internet LED, Mitrastar Jayden, 20130308*/ 			
			 //kerSysLedCtrl(kLedWanData, kLedStateOff);
			 kerSysLedCtrl(kLedWanData, kLedStateOn);
	#endif
			kerSysLedCtrl(kLedAdsl_TO2, kLedStateOn);
	kerSysSendtoMonitorTask(MSG_NETLINK_BRCM_LINK_STATUS_CHANGED|0x1,NULL,0);
#if 1 // for Internet LED
			printk("link change\n");
			if (isDataTrafficForwarded == NULL)
			{
				printk("set isDataTrafficForwarded\n");
				isDataTrafficForwarded = bcmenet_dataforwarded;
			}
#endif
		}
		else{/*eth4 DOWN*/
			kerSysSendtoMonitorTask(MSG_NETLINK_BRCM_LINK_STATUS_CHANGED|0x4,NULL,0);
			/*LED, chris 20130523*/
			kerSysLedCtrl(kLedAdsl_TO2, kLedStateOff);
#if 1 // for Internet LED
			kerSysLedCtrl(kLedWanData, kLedStateOff);
			if (isDataTrafficForwarded == bcmenet_dataforwarded)
			{
				printk("set isDataTrafficForwarded to NULL\n");
...
int bcmenet_dataforwarded(void)
{
	...
	ethsw_rreg_ext(PAGE_MIB_P0 + 3, REG_MIB_P0_TXUPKTS,
               (uint8_t *)&ctr32, 4, 0);                        // Get TX unicast packet count
        TxCounter = (uint64)ctr32;                                          // Keep running count        

        ethsw_rreg_ext(PAGE_MIB_P0 + 3, REG_MIB_P0_TXMPKTS,
               (uint8_t *)&ctr32, 4, 0);                        // Get TX multicast packet count
        tempctr64 = (uint64)ctr32;
        TxCounter += tempctr64;                                             // Keep running count
        
        ethsw_rreg_ext(PAGE_MIB_P0 + 3, REG_MIB_P0_TXBPKTS,
               (uint8_t *)&ctr32, 4, 0);                        // Get TX broadcast packet count
        tempctr64 = (uint64)ctr32;
        TxCounter += tempctr64;
		tmpTxCoutner = TxCounter - VoIPTxCounter;
		if (tmpTxCoutner != lastTxCounter)
		{
			ret = 1;
		}
		lastTxCounter = tmpTxCoutner;

		//printk("bcmenet_dataforwarded ret %d, txcounter %10lu, voipCounter %lu\n", ret, (long unsigned int)lastTxCounter, VoIPTxCounter); 
	return ret;
...
int hpnaReset(void) {
  //unsigned long flags;
  unsigned short hpnaResetGpio;
  int rc;

  /* hard reset the HPNA */
  printk("Resetting HPNA...");

  BCM_ASSERT(GPIO);
  rc = BpGetHpnaResetGpio(&hpnaResetGpio);
  BCM_ASSERT(rc == BP_SUCCESS);

  /* PULL LOW HPNA RESET pin */
  kerSysSetGpioState(hpnaResetGpio, kGpioActive);

  msleep(10);

  /* PULL HIGH HPNA RESET pin */
  kerSysSetGpioState(hpnaResetGpio, kGpioInactive);
