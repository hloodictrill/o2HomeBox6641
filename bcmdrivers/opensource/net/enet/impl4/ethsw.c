...
int ethsw_reset_ports(struct net_device *dev)
{
    ...
    for (i = 0; i < NUM_RGMII_PORTS; i++) {
...
#endif    
        {
            ...
            v8 |= REG_RGMII_CTRL_TIMING_SEL;
#ifdef BUILD_5G /* Support Quantenna 5GHz Wifi module on RGMII __TELEFONICA__, MTSC Curtis, 20111212. */
...
/*Configure port state for HPAN module, KeyYang@MSTC 20120914*/
		...
		if(port == 5){
			...
			/*Port State
				...
				*/
			v16 = 0x7f ;
			extsw_wreg_wrap(PAGE_CTRL, REG_CTRL_OVERRIDE+4, (uint8 *)&v16, 2);
		}
#endif
