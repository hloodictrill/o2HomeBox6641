...
#if 1
#include <bcmnetlink.h>				/* for kerSysSendtoMonitorTask */
...
/* MSTC , rewrite "RaynorChung: Ssupport USB LED " to compiler flag by Andylin 20120521*/
#ifdef ENABLE_USB_STORAGE_LED
extern void ZyXEL_USB_LED_Control ( int USBId, int status );
#endif
...
/*MSTC , rewrite "RaynorChung: Ssupport USB LED " to compiler flag by Andylin 20120521*/
#ifdef ENABLE_USB_STORAGE_LED 
		ZyXEL_USB_LED_Control( usblp->dev->portnum, 0x00 );
#endif
...
/*MSTC , rewrite "RaynorChung: Ssupport USB LED " to compiler flag by Andylin 20120521*/
	...
	#ifdef USB_PORT_SWAP /* MSTC, USB PORT SWAP by Andylin 20120509*/
		if(usblp->dev->portnum == 2) 
	#else
		if(usblp->dev->portnum == 1) 
	#endif
		{
#ifndef MTS_TELEFONICA_WWAN
			/* usb 0 will control by 3G */
			kerSysLedCtrl(kLedUSB0,kLedStateFastBlinkContinues);
#endif /*MTS_TELEFONICA_WWAN*/
		} 
	#ifdef USB_PORT_SWAP /* MSTC, USB PORT SWAP by Andylin 20120509*/
		else if(usblp->dev->portnum == 1) 
	#else
...
/*[IreneWang@MSTC:TO2]$ Maintain the LPD printer queueName and deviceNode + Enhance the kernel message length, 20130801 + 20130806*/
#if 1
	snprintf(usbPrinterMsg, sizeof(usbPrinterMsg), "r|n=%d|p=%s|s=%s|m=%s", intf->minor, dev->product, dev->serial, dev->manufacturer);
	kerSysSendtoMonitorTask(MSG_NETLINK_USB_PRINTER_INFO, usbPrinterMsg, strlen(usbPrinterMsg));
#endif  /*1*/
...
/*MSTC , rewrite "RaynorChung: Ssupport USB LED " to compiler flag by Andylin 20120521 */
#ifdef ENABLE_USB_STORAGE_LED 
		ZyXEL_USB_LED_Control( usblp->dev->portnum, 0x01 );
#endif
...
#if 1
	char usbPrinterMsg[256] = {'\0'};
	snprintf(usbPrinterMsg, sizeof(usbPrinterMsg), "d|n=%d|p=%s|s=%s|m=%s", intf->minor, usblp->dev->product, usblp->dev->serial, usblp->dev->manufacturer);
	kerSysSendtoMonitorTask(MSG_NETLINK_USB_PRINTER_INFO, usbPrinterMsg, strlen(usbPrinterMsg));
#endif  /*1*/
