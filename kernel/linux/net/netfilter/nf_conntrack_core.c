...
//<< asdrick
    // Workround : check conntrack pointer is vaild or not .
    if(     (((unsigned long)ct & 0x80000001) != 0x80000000 ) //check ct is a vaild address
        ||  (ct->timeout.data != (unsigned long)ct)
        ||  ( (0x864000fc)    == (unsigned long)ct)
        ||  ( (0x864000f0)    == (unsigned long)ct)        
      )

    {
        if(net_ratelimit())
        {
            printk(KERN_EMERG"%s:%d invaild ct:%p\n",__FUNCTION__,__LINE__,ct);
        }        
        return;
    }
//>> asdrick
