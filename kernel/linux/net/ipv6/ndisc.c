...
/*KeyYang@MSTC: Merge from Telefonica_Common, 20120710*/
		...
		{
		sprintf(defrtaddr1, "%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x",
	        ipv6_hdr(skb)->saddr.s6_addr16[0], ipv6_hdr(skb)->saddr.s6_addr16[1],
	        ipv6_hdr(skb)->saddr.s6_addr16[2], ipv6_hdr(skb)->saddr.s6_addr16[3],
	        ipv6_hdr(skb)->saddr.s6_addr16[4], ipv6_hdr(skb)->saddr.s6_addr16[5],
	        ipv6_hdr(skb)->saddr.s6_addr16[6], ipv6_hdr(skb)->saddr.s6_addr16[7]);
		snprintf(cmd1, sizeof(cmd1), "%s|%s", skb->dev->name, defrtaddr1);
		kerSysSendtoMonitorTask(MSG_NETLINK_BRCM_IPV6_DEFAULT_ROUTER, cmd1, strlen(cmd1)+1);
		}
#endif /*TELEFONICA*/
