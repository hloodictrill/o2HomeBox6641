...
void addrconf_prefix_rcv(struct net_device *dev, u8 *opt, int len)
{
	...
	if (pinfo->onlink) {
		...
		if (rt && addrconf_is_prefix_route(rt)) {
			...
			/* MSTC , fix ipv6 add routing table on layer 2 interface by Andylin */
			if(strstr(dev->name,".") != 0){
...
ok:
		...
		if (ifp == NULL && valid_lft) {
			...
			#if 1 /*David SUPPORT_TELEFONICA*/
			/*if add RA address success, we send event to userspace for update external address*/
			snprintf(cmd, sizeof(cmd), "%s|%04x:%04x:%04x:%04x:%04x:%04x:%04x:%04x|%u", dev->name,
					addr.s6_addr16[0], addr.s6_addr16[1],
					addr.s6_addr16[2], addr.s6_addr16[3],
					addr.s6_addr16[4], addr.s6_addr16[5],
					addr.s6_addr16[6], addr.s6_addr16[7], pinfo->prefix_len);
			kerSysSendtoMonitorTask(MSG_NETLINK_BRCM_IPV6_RA_PREFIX_ADDR, cmd, strlen(cmd)+1);
			#endif
