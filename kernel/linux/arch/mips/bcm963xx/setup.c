...
#endif
#ifdef BUILD_5G
	unsigned short Gpio5GPwrCtl;
	unsigned short Gpio5GReset;
#endif
...
/*KeyYang@MSTC: Merge from Telefonica_Common, 20120702*/
        ...
        /*__MTS__, RaynorChung: Support DSL-491HNU-B1B v2 board*/
...
#endif
/*KeyYang@MSTC: Merge from Telefonica_Common, 20120702*/
#ifdef CONFIG_BUILD_GPHY_SPD
        /*__MTS__, RaynorChung: Support DSL-491HNU-B1B v2 board*/
        LED->ledHWDis &= ~(1 << LED_GPHY0_SPD0);
#if 0	/* Jayden@MSTC for To2 disable power red led light when ethwan connected */
...
/*KeyYang@MSTC: Merge from Telefonica_Common, 20120702*/
        ...
        if ( BpGetWanDataLedGpio(&gpio) == BP_SUCCESS ) {
            if ((gpio & BP_GPIO_NUM_MASK) == LED_INET_ACT) {
		#if 1	/*ChenHe@MSTC for To2 DSL Data ACT*/
		LED->ledHWDis &= ~(1 << LED_INET_ACT);
		#endif
...
/*KeyYang@MSTC: Merge from Telefonica_Common, 20120702*/
...
#if 1
        //printk("\r\n\r\n##### Set GPIO 21 to HIGH in kernel!!! #####\r\n");
        GPIO->GPIODir |= GPIO_NUM_TO_MASK(21);
        GPIO->GPIOio &= ~GPIO_NUM_TO_MASK(21);
        msleep(300);
        GPIO->GPIOio |= GPIO_NUM_TO_MASK(21);
#endif
	...
	if(BpGet5GResetGpio(&Gpio5GReset) == BP_SUCCESS) {
		printk("set GPIO %d to HIGH\r\n", Gpio5GReset);
		GPIO->GPIODir |= GPIO_NUM_TO_MASK(Gpio5GReset);
		GPIO->GPIOio |= GPIO_NUM_TO_MASK(Gpio5GReset);
	}
	if(BpGet5GPwrCtlGpio(&Gpio5GPwrCtl) == BP_SUCCESS) {
		printk("set GPIO %d to HIGH\r\n", Gpio5GPwrCtl);
		GPIO->GPIODir |= GPIO_NUM_TO_MASK(Gpio5GPwrCtl);
		GPIO->GPIOio |= GPIO_NUM_TO_MASK(Gpio5GPwrCtl);
	}
#endif

#ifdef CONFIG_BUILD_FW_WITH_DIFFERENT_HW
	if(GPIO->GPIOio & GPIO_NUM_TO_MASK(BP_GPIO_8_AH)) {
		if(GPIO->GPIOio & GPIO_NUM_TO_MASK(BP_GPIO_14_AH)) {
			printk("##### kernel : device contain 5G module  #####\r\n");
			BpSetHWType(HW_5G_IAD);
		} else {
			printk("##### kernel : device contain HPNA module #####\r\n");
			BpSetHWType(HW_HPNA_IAD);
		}
	} else {
		printk("##### kernel : device is pure IAD #####\r\n");
		BpSetHWType(HW_PURE_IAD);
	}
#endif
